...
The current code supports the active Paystack Dashboard plans: Basic (`₦2,500`), Premium (`₦4,500`), and Business (`₦7,000`) monthly. Before a real paid checkout can succeed, Vercel must have `SUPABASE_SERVICE_ROLE_KEY`, `PAYSTACK_BASIC_PLAN_CODE`, `PAYSTACK_PREMIUM_PLAN_CODE`, and `PAYSTACK_BUSINESS_PLAN_CODE` in addition to `PAYSTACK_SECRET_KEY`. Those secret values are not stored in the repository. Build and Node syntax checks passed after the integration; the code is ready for deployment once the missing environment configuration is added.
