...
export const PLANS = {
  basic: { name: 'Basic', amountNaira: 2999, amountKobo: 299900, env: 'PAYSTACK_BASIC_PLAN_CODE', listingLimit: 15 },
  premium: { name: 'Premium', amountNaira: 7000, amountKobo: 700000, env: 'PAYSTACK_PREMIUM_PLAN_CODE', listingLimit: 35 },
  business: { name: 'Business', amountNaira: 14999, amountKobo: 1499900, env: 'PAYSTACK_BUSINESS_PLAN_CODE', listingLimit: 60 },
};
...
export function getPaystackSecret() {
  ...
  const secretKey = typeof rawKey === 'string' ? rawKey.trim().replace(/^['\"]|['\"]$/g, '') : '';
  if (!secretKey) throw new Error('Paystack server configuration is incomplete.');
  if (/^pk_(test|live)_/.test(secretKey)) throw new Error('Paystack public key was supplied as the server secret key. Use the sk_test_ or sk_live_ key in Vercel.');
  if (!/^sk_(test|live)_/.test(secretKey)) throw new Error('Paystack secret key format is invalid. Use the full sk_test_ or sk_live_ key from Paystack.');
  return secretKey;
}
...
export function callbackUrl(req) {
  return process.env.PAYSTACK_CALLBACK_URL || `${process.env.APP_URL || 'https://bese26.shop'}/?payment=paystack`;
}
...
export function assertPaymentOwner(providerData, userId) {
  if (!userId || providerData?.metadata?.user_id !== userId) {
