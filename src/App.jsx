...
function PublicBusinessPage({ handle }) {
  ...
  useEffect(() => { let mounted = true; fetchPublicBusiness(handle).then((data) => mounted && setState({ loading: false, data, error: '' })).catch((error) => mounted && setState({ loading: false, data: null, error: error.message || 'Unable to load this business.' })); return () => { mounted = false; }; }, [handle]);
  ...
  if (state.error || !state.data) return <div className="public-business-shell"><section className="public-business-not-found"><div className="brand-mark">B</div><div className="eyebrow">BESE44 SHOP</div><h1>Business not found</h1><p>This public business profile does not exist, is inactive, or has not been verified yet.</p><a className="primary-button" href="https://bese26-app.vercel.app/">Back to Bese26 <ArrowRight size={16} /></a></section></div>;
