...
function PublicPersonalPage({ data }) {
  ...
  return <div className="public-business-shell"><header className="public-business-topbar"><a href="https://bese26.shop/" className="public-brand"><img className="brand-mark-image" src="/images/bese26-logo-icon.png" alt="Bese26" /><strong>Bese26<span>.shop</span></strong></a><button type="button" className="secondary-button" onClick={share}>Share profile</button></header><main className="public-business-main"><section className="public-business-hero"><div className="public-business-logo">{profile.avatar_path ? <img src={getAvatarUrl(profile.avatar_path)} alt={`${profile.display_name} profile`} /> : <span>{profile.display_name.slice(0, 1).toUpperCase()}</span>}</div><div className="public-business-identity"><div className="eyebrow">PUBLIC PERSONAL PROFILE</div><h1>{profile.display_name}</h1><strong className="public-business-handle">@{profile.username}</strong>{profile.is_verified && <span className="verified-badge"><BadgeCheck size={13} /> Verified seller</span>}<p>{profile.bio || 'This Bese26 member has not added a public bio yet.'}</p><span className="public-business-location"><MapPin size={14} /> {location || 'Nigeria'}</span><div className="public-business-actions"><button type="button" className="primary-button" onClick={() => window.location.href = 'https://bese26.shop/'}>Open Bese26 <ArrowRight size={15} /></button><button type="button" className="secondary-button" onClick={share}>Share</button></div></div></section><section className="public-business-stats"><div><strong>{listings.length}</strong><span>Active listings</span></div><div><strong>{profile.account_type || 'Seller'}</strong><span>Account type</span></div><div><strong>{profile.seller_rating ? `${Number(profile.seller_rating).toFixed(1)}/5` : '—'}</strong><span>{profile.seller_rating_count || 0} reviews</span></div></section><section className="public-business-listings"><div className="section-heading"><div><div className="eyebrow">AVAILABLE NOW</div><h2>Listings by {profile.display_name}</h2></div><span>{listings.length} listing{listings.length === 1 ? '' : 's'}</span></div>{listings.length ? <div className="product-grid">{listings.map((listing) => <a className="product-card" key={listing.id} href="https://bese26.shop/"><div className="product-image-wrap">{listing.image ? <img src={listing.image} alt={listing.title} className="product-image" /> : <div className="product-image-placeholder"><Package size={26} /></div>}</div><div className="product-info"><div className="product-price">{listing.price}</div><h3>{listing.title}</h3><div className="product-meta"><MapPin size={13} /> {listing.location}</div><div className="product-foot"><span>{listing.condition}</span><span>{listing.posted}</span></div></div></a>)}</div> : <div className="empty-state"><Package size={26} /><h3>No active listings</h3><p>This member has not published any approved listings yet.</p></div>}</section></main><footer className="public-business-footer">Powered by Bese26.shop · Marketplace by Bese26</footer></div>;
}
...
function PublicListingRoute({ listingId }) {
  ...
  return <ListingModal listing={listing} onClose={() => window.location.assign('/')} onDemoAction={(message) => window.alert(message)} />;
}
