  ...
  return <div className="listing-modal-backdrop listing-details-backdrop" role="dialog" aria-modal="true" aria-label="Listing details" onClick={(event) => event.target === event.currentTarget && onClose?.()}>
    <div className="listing-details-view">
      ...
      <main className="listing-new-content">
        ...
        <section className="listing-new-section listing-new-chat"><div className="listing-new-section-heading"><div><span className="listing-new-kicker">PRIVATE CONVERSATION</span><h2>Chat with the seller</h2></div><MessageCircle size={20} /></div><div className="listing-new-quick-actions"><button type="button" onClick={() => setQuickMessage('Is this still available?')}>Is this available?</button><button type="button" onClick={() => setQuickMessage('What is your final price?')}>Last price</button><button type="button" onClick={() => onStartChat?.(listing, 'offer')}>Make an offer</button></div><textarea value={quickMessage} onChange={(event) => setQuickMessage(event.target.value)} placeholder="Write your message here…" maxLength={500} /><button type="button" className="listing-new-start-chat" onClick={() => onStartChat?.(listing, 'message', quickMessage)}>Start chat <ArrowRight size={16} /></button></section>
        ...
        {deliveryLabels.length > 0 && <section className="listing-new-section"><div className="listing-new-section-heading"><h2>Delivery</h2></div><div className="listing-new-delivery">{deliveryLabels.map((item) => <span key={item}>✓ {item}</span>)}</div></section>}
        ...
        <section className="listing-new-section listing-new-reviews"><div className="listing-new-section-heading"><h2>Feedback about seller</h2><span>{reviews.length ? `View all (${reviews.length})` : '0 reviews'}</span></div>{reviews.length ? reviews.slice(0, 2).map((review) => <article className="listing-new-review" key={review.id}><div className="listing-new-review-top"><div className="listing-new-review-avatar">{(review.reviewer?.display_name || 'B').slice(0, 1).toUpperCase()}</div><strong>{review.reviewer?.display_name || 'Buyer'}</strong><time>{new Date(review.created_at).toLocaleDateString('en-NG', { day: 'numeric', month: 'short' })}</time></div><div className="listing-new-stars">{'★'.repeat(Math.max(0, Math.min(5, Number(review.rating) || 0)))}</div><p>{review.body || 'No written review.'}</p></article>) : <div className="listing-new-no-reviews"><span>☆</span><p>No reviews yet</p></div>}</section>
    ...
    {reportOpen && <div className="listing-report-overlay" role="dialog" aria-label="Report listing" onClick={(event) => event.target === event.currentTarget && setReportOpen(false)}><form className="listing-report-card" onSubmit={submitReport}><button type="button" className="icon-button" onClick={() => setReportOpen(false)} aria-label="Close report"><X size={18} /></button><h2>Report listing</h2><label>Reason<select value={reportReason} onChange={(event) => setReportReason(event.target.value)}>{['scam', 'fake_product', 'counterfeit', 'prohibited_item', 'wrong_information', 'fake_identity', 'harassment', 'suspicious_activity', 'other'].map((reason) => <option key={reason} value={reason}>{reason.replaceAll('_', ' ')}</option>)}</select></label><label>Details<textarea value={reportDetails} onChange={(event) => setReportDetails(event.target.value)} maxLength={1500} placeholder="Tell Bese26 moderation what happened" /></label><button className="primary-button" disabled={actionBusy}>{actionBusy ? 'Submitting…' : 'Submit report'}</button></form></div>}
  </div>;
}
