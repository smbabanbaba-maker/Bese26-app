...
function FirstVisitCard({ user, onNavigate }) {
  ...
  if (!user && !done.intent) return <section className="first-visit-card" aria-label="Choose how to use Bese26"><div><div className="eyebrow">WELCOME TO BESE26</div><h2>What do you want to do today?</h2><p>Browse approved listings or start selling in a few simple steps.</p></div><div className="first-visit-actions"><button type="button" className="primary-button" onClick={() => update('intent', () => onNavigate('search'))}><ShoppingBag size={16} /> I want to buy</button><button type="button" className="secondary-button" onClick={() => update('intent', () => onNavigate('sell'))}><Plus size={16} /> I want to sell</button></div><button type="button" className="text-action first-visit-dismiss" onClick={() => { setDismissed(true); try { window.localStorage.setItem('bese26:onboarding-dismissed', '1'); } catch {} }}>Skip for now</button></section>;
  if (!user) return null;
  const items = [{ key: 'profile', label: 'Complete your profile', note: 'Add your name and location', action: () => onNavigate('profile') }, { key: 'browse', label: 'Save a listing', note: 'Keep useful items for later', action: () => onNavigate('search') }, { key: 'sell', label: 'Post your first item', note: 'Reach buyers near you', action: () => onNavigate('sell') }];
  if (items.every((item) => done[item.key])) return null;
  return <section className="onboarding-checklist" aria-label="Getting started checklist"><div className="onboarding-heading"><div><div className="eyebrow">GETTING STARTED</div><h2>Three steps to get value from Bese26</h2><p>You can return to this checklist from your Profile.</p></div><button type="button" className="icon-button" aria-label="Dismiss getting started" onClick={() => { setDismissed(true); try { window.localStorage.setItem('bese26:onboarding-dismissed', '1'); } catch {} }}><X size={16} /></button></div><div className="onboarding-items">{items.map((item) => <button type="button" className={`onboarding-item ${done[item.key] ? 'done' : ''}`} key={item.key} onClick={() => update(item.key, item.action)}><span>{done[item.key] ? <Check size={15} /> : item.key === 'profile' ? <UserRound size={15} /> : item.key === 'browse' ? <Heart size={15} /> : <Plus size={15} />}</span><strong>{item.label}</strong><small>{done[item.key] ? 'Completed' : item.note}</small><ChevronRight size={15} /></button>)}</div></section>;
}
function HomeView({ user, marketListings, adCampaigns = [], onOpenListing, savedIds, onToggleSave, onSearch, onNavigate, onShowNotifications }) {
  ...
  return (
    <div className="page-stack home-page">
      <FirstVisitCard user={user} onNavigate={onNavigate} />
...
function ListingModal({ listing, user, onClose, isSaved, onToggleSave, onDemoAction, onAuthRequired, onStartChat, onEditListing, onOpenListing }) {
  ...
  useEffect(() => { if (!listing) return undefined; setActiveImage(0); setZoomed(false); setExpandedDescription(false); setReportOpen(false); setContact({ phone: '', whatsapp: '' }); setContactLoading(true); setLoadingDetails(true); Promise.allSettled([fetchListingReviews(listing.id), fetchSimilarListings(listing), fetchListingContact(listing.id)]).then(([reviewResult, similarResult, contactResult]) => { if (reviewResult.status === 'fulfilled') setReviews(reviewResult.value || []); if (similarResult.status === 'fulfilled') setSimilar(similarResult.value || []); if (contactResult.status === 'fulfilled') setContact(contactResult.value || { phone: '', whatsapp: '' }); }).finally(() => { setLoadingDetails(false); setContactLoading(false); }); recordListingView(listing.id).catch(() => {}); return undefined; }, [listing?.id]);
