...
function NotificationsView({ user, onAuthRequired, onBack, onNotice, onNavigate, onOpenListing }) {
  ...
  useEffect(() => { let mounted = true; if (!user) { setLoading(false); return undefined; } fetchNotifications(user.id).then((rows) => { if (mounted) setItems(rows || []); }).catch((error) => mounted && onNotice?.(error.message || 'Could not load notifications.')).finally(() => mounted && setLoading(false)); Promise.all([fetchSellerEntitlement(), fetchMyListings({ sellerId: user.id, status: 'active' }), fetchMyBoosts(user.id)]).then(([access, listings, activeBoosts]) => { if (!mounted) return; setEntitlement(access || null); setSellerListings(listings || []); setBoosts((activeBoosts || []).filter((boost) => ['active', 'pending', 'scheduled'].includes(boost.status))); }).catch(() => {}); const unsubscribe = subscribeToNotifications(user.id, (payload) => { if (!mounted || !payload?.new) return; setItems((current) => [payload.new, ...current.filter((item) => item.id !== payload.new.id)].slice(0, 50)); }); return () => { mounted = false; unsubscribe?.(); }; }, [user, onNotice]);
...
function SearchView({ marketListings, categories, search, setSearch, onOpenListing, savedIds, onToggleSave, onBack, adCampaigns = [] }) {
  ...
  return (
    <div className="page-stack search-page">
      ...
      <div className="search-box large-search"><Search size={19} /><input type="search" autoFocus value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search phones, cars, fashion or Kano…" /><button className="search-clear" onClick={() => setSearch('')} aria-label="Clear search" disabled={!search}><X size={16} /></button></div>
      ...
      <div className="search-result-head"><span>Recommended listings</span><select value={sort} onChange={(e) => setSort(e.target.value)} aria-label="Sort listings"><option>Recommended</option><option>Newest</option><option>Price low → high</option><option>Price high → low</option></select></div>
  ...
  return <div className="listing-modal-backdrop listing-details-backdrop" role="dialog" aria-modal="true" aria-label="Listing details" onClick={(event) => event.target === event.currentTarget && onClose?.()}>
    <div className="listing-details-view">
      ...
      <main className="listing-new-content">
        ...
        <section className="listing-new-section listing-new-contact"><div className="listing-new-action-grid"><button type="button" className="listing-new-outline-action" onClick={requestCallback} disabled={actionBusy}><Phone size={16} /> Request callback</button><button type="button" className="listing-new-solid-action" onClick={callSeller} disabled={contactLoading || !sellerPhone}><Phone size={16} /> {contactLoading ? 'Loading…' : 'Call'}</button></div>{whatsapp && <button type="button" className="listing-new-whatsapp" onClick={openWhatsApp} disabled={contactLoading}><MessageCircle size={15} /> WhatsApp seller</button>}</section>
        ...
        {delivery.length > 0 && <section className="listing-new-section"><div className="listing-new-section-heading"><h2>Delivery</h2></div><div className="listing-new-delivery">{delivery.map((item) => <span key={String(item)}>{String(item).replaceAll('_', ' ')}</span>)}</div></section>}
        <section className="listing-new-section listing-new-seller"><div className="listing-new-seller-head"><div className="listing-new-seller-avatar">{listing.sellerAvatar ? <img src={listing.sellerAvatar} alt="" /> : listing.sellerInitials}</div><div><h2>{listing.sellerDisplayName || listing.seller}</h2><div className="listing-new-badges">{listing.idVerified && <span className="id-verified"><CheckCircle2 size={13} /> ID Verified</span>}{listing.cacVerified && <span className="cac-verified"><CheckCircle2 size={13} /> CAC Verified</span>}</div><p><MapPin size={13} /> {sellerLocation} · Member since {sellerSince}</p></div><button type="button" className="listing-new-link" onClick={() => listing.sellerBusinessHandle ? window.location.assign(`/@${listing.sellerBusinessHandle}`) : listing.sellerId && raw.profiles?.username ? window.location.assign(`/${raw.profiles.username}`) : onDemoAction?.('Seller profile is not public yet.')}>View store <ChevronRight size={15} /></button></div><div className="listing-new-seller-actions"><button type="button" onClick={followSeller} disabled={owner || !listing.sellerId}><UserPlus size={15} /> {followingSeller ? 'Following' : 'Follow'}</button><button type="button" onClick={share}><Share2 size={15} /> Share</button></div></section>
...
function PublicFollowShowcase({ targetId, name }) {
  ...
  return <section className="public-follow-showcase" aria-labelledby="public-follow-title"><div className="public-follow-heading"><div><div className="eyebrow">COMMUNITY AROUND THIS PROFILE</div><h2 id="public-follow-title">Stay connected with {name}</h2><p>Follow updates, new listings, and trusted activity from this Bese26 profile.</p></div><button type="button" className={`public-follow-button ${isFollowing ? 'following' : ''}`} onClick={handleFollow} disabled={busy || sessionUser?.id === targetId}><UserPlus size={15} /> {isFollowing ? 'Following' : 'Follow'}</button></div><div className="public-follow-stats"><div><strong>{summary.followers}</strong><span>Followers</span></div><div><strong>{summary.following}</strong><span>Following</span></div><div><strong>{summary.followers + summary.following}</strong><span>Community links</span></div></div><div className="public-follow-columns"><div className="public-follow-list"><div className="public-follow-list-heading"><strong>People following</strong><span>{summary.followers}</span></div>{followers.length ? followers.map(person) : <p className="public-follow-empty">Be the first person to follow this profile.</p>}</div><div className="public-follow-list"><div className="public-follow-list-heading"><strong>{name} follows</strong><span>{summary.following}</span></div>{following.length ? following.map(person) : <p className="public-follow-empty">No public following yet.</p>}</div></div></section>;
}
...
function PublicProfileHeader({ profile, business, listings, share }) {
  ...
  return <section className="public-business-hero storefront-hero-clean"><div className="public-hero-brand-mark" aria-hidden="true"><img src="/images/bese26-logo-icon.png" alt="" /></div><button type="button" className="public-hero-share" onClick={share} aria-label="Share shop"><Share2 size={14} /> Share</button><div className="public-business-logo">{avatar ? <img src={getAvatarUrl(avatar)} alt={`${name} profile`} /> : <span>{name.slice(0, 1).toUpperCase()}</span>}</div><div className="public-business-logo-location"><span className="public-business-location"><MapPin size={14} /> {location || 'Nigeria'}</span></div><div className="public-business-identity"><div className="eyebrow">{isBusiness ? 'PUBLIC BUSINESS' : 'PUBLIC SELLER PROFILE'}</div><h1 className="public-business-name">{name}{(business?.is_verified || profile?.is_verified) && <span className="public-name-verified" aria-label={business?.is_verified ? 'Verified business' : 'Verified seller'} title={business?.is_verified ? 'Verified business' : 'Verified seller'}><BadgeCheck size={32} strokeWidth={3} /></span>}</h1><div className="public-business-subline">{(business?.category || business?.business_type) && <span className="public-business-category">{business.category || business.business_type}</span>}</div>{safeDescription && <p>{safeDescription}</p>}<button type="button" className="public-browse-cta" onClick={scrollToListings}>View listings <ArrowRight size={14} /></button></div><div className="public-business-trust">{(business?.is_verified || profile?.is_verified) ? <span className="verified-badge"><BadgeCheck size={14} /> <span>Verified</span><small>{business?.is_verified ? 'Business' : 'Seller'}</small></span> : <span className="unverified-badge">Public profile</span>}<div className="public-business-stats"><span><strong>{listings.length}</strong><small>Listings</small></span><span><strong>{profile?.seller_rating ? Number(profile.seller_rating).toFixed(1) : 'New'}</strong><small>Rating</small></span></div></div></section>;
}
