...
function VerificationBadges({ idVerified = false, cacVerified = false, compact = false }) {
  if (!idVerified && !cacVerified) return null;
  return <span className={`verification-badge-group ${compact ? 'verification-badge-group-compact' : ''}`} aria-label="Seller verification"><span className="verification-id-badge" title="Identity Verified — Bese26 has verified this seller's identity."><BadgeCheck size={13} /> ID Verified</span>{cacVerified && <span className="verification-cac-badge" title="Business Verified — This seller's business registration has been verified by Bese26."><BadgeCheck size={13} /> CAC Verified</span>}</span>;
}
...
function ListingModal({ listing, user, onClose, isSaved, onToggleSave, onDemoAction, onAuthRequired, onStartChat, onEditListing, onOpenListing }) {
  ...
  const identityVerified = Boolean(listing.idVerified || raw.profiles?.is_verified);
  const cacVerified = Boolean(listing.cacVerified || raw.business_profile?.is_verified);
