  ...
  return (
    <div className="page-stack home-page">
      ...
      {promo && <section style={promo.image_only ? { aspectRatio: `${promo.creative_width} / ${promo.creative_height}` } : undefined} className={`home-ad-banner ${promo.type === 'dashboard' ? 'home-ad-dashboard dashboard-welcome-card' : `home-ad-slide-${promoIndex}`}`} aria-label={promo.type === 'dashboard' ? 'Your Bese26 dashboard' : 'Sponsored promotion'}>
        {promo.type === 'dashboard' ? <><div className="dashboard-welcome-copy"><div className="eyebrow">{user ? 'YOUR BESE26 DASHBOARD' : 'WELCOME TO BESE26'}</div><h1>{user ? <>Good to see you, <span>{displayName}</span>.</> : <>Buy and sell with confidence.</>}</h1><p>{user ? 'Pick up where you left off and keep your marketplace moving.' : 'Browse trusted listings or start selling in a few simple steps.'}</p><div className="dashboard-actions"><button type="button" className="dashboard-primary-action" onClick={() => onNavigate(user ? 'sell' : 'search')}><Plus size={15} /> {user ? 'List an item' : 'Browse listings'}</button><button type="button" className="dashboard-secondary-action" onClick={() => onNavigate(user ? 'profile' : 'sell')}>{user ? 'View profile' : 'Start selling'} <ArrowRight size={15} /></button></div></div><div className="dashboard-orbit-art" aria-hidden="true"><span className="dashboard-orbit dashboard-orbit-one" /><span className="dashboard-orbit dashboard-orbit-two" /><img src="/images/bese26-logo-icon.png" alt="" /></div><div className="dashboard-metrics"><span><strong>{marketListings.length}</strong><small>live listings</small></span><span><strong>{savedIds.length}</strong><small>saved items</small></span><span><strong>100%</strong><small>secure access</small></span></div></> : promo.image_only ? promo.cta_target?.trim() ? <button type="button" className="home-ad-image-only" onClick={promo.onAction} aria-label={promo.title}><img src={promo.image_url} alt={promo.title} /></button> : <div className="home-ad-image-only" aria-label={promo.title}><img src={promo.image_url} alt={promo.title} /></div> : <><div className="home-ad-copy"><div className="eyebrow light">{promo.eyebrow} <span className="home-ad-sponsored">Sponsored space</span></div><h2>{promo.title}</h2><p>{promo.body}</p><button type="button" className="home-ad-cta" onClick={promo.onAction}>{promo.action} <ArrowRight size={15} /></button></div><div className={`home-ad-art ${promo.type === 'ad' ? 'home-ad-art-clickable' : ''}`} aria-hidden={promo.type !== 'ad'} onClick={promo.type === 'ad' ? promo.onAction : undefined} role={promo.type === 'ad' ? 'link' : undefined} tabIndex={promo.type === 'ad' ? 0 : undefined} onKeyDown={promo.type === 'ad' ? (event) => { if (event.key === 'Enter' || event.key === ' ') promo.onAction(); } : undefined}><img src={promo.image_url || "/images/bese26-official-logo.png"} alt={promo.type === 'ad' ? `${promo.title} advertisement` : ''} /></div></>}
      ...
      <div className="home-policy-links" aria-label="Bese26 information"><a href="/#about">About</a><a href="/#safety">Safety</a><a href="/#terms">Terms</a><a href="/#privacy">Privacy</a><a href="/#refund-policy">Refunds</a></div>
    </div>
  );
}
