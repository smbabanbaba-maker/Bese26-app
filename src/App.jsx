...
function SearchView({ marketListings, categories, search, setSearch, onOpenListing, savedIds, onToggleSave, onBack, adCampaigns = [] }) {
  ...
  return (
    <div className="page-stack search-page">
      <div className="back-row"><button className="icon-button" onClick={onBack} aria-label="Back"><ArrowLeft size={18} /></button><span>Bincika kayayyaki</span></div>
      <div className="page-title-row"><div><h1>Bincika kayayyaki</h1><p>Nemo abin da kake bukata cikin sauri.</p></div><div className="results-count">{filtered.length} results</div></div>
      <div className="search-box large-search"><Search size={19} /><input type="search" autoFocus value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Nemo phones, cars, fashion ko Kano…" /><button className="search-clear" onClick={() => setSearch('')} aria-label="Clear search" disabled={!search}><X size={16} /></button></div>
      ...
      <div className="search-result-head"><span>Kayayyakin da suka dace</span><select value={sort} onChange={(e) => setSort(e.target.value)} aria-label="Sort listings"><option>Recommended</option><option>Newest</option><option>Price low → high</option><option>Price high → low</option></select></div>
