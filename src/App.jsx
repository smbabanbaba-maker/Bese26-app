...
function MessagesView({ user, liveListing, onDemoAction, initialMessageId, onSelectConversation, onBackToInbox }) {
...
function PublicBusinessPage({ handle }) {
  ...
  useEffect(() => { let mounted = true; if (!supabase) return undefined; supabase.auth.getSession().then(({ data }) => mounted && setViewer(data?.session?.user || null)); return () => { mounted = false; }; }, []);
  useEffect(() => { let mounted = true; const targetId = state.data?.business?.profile_id; if (!targetId || !viewer?.id || viewer.id === targetId) { setFollowState((current) => ({ ...current, following: false })); return undefined; } getFollowState(viewer.id, targetId).then((data) => mounted && setFollowState(data)).catch(() => {}); return () => { mounted = false; }; }, [state.data, viewer?.id]);
  ...
  if (state.error || !state.data) return <div className="public-business-shell"><section className="public-business-not-found"><img className="brand-mark-image" src="/images/bese26-official-logo.png" alt="Bese26" /><div className="eyebrow">BESE26 SHOP</div><h1>Public profile not found</h1><p>This store does not exist or is not currently active.</p><a className="primary-button" href="https://www.bese26.shop/">Back to Bese26 <ArrowRight size={16} /></a></section></div>;
  if (state.data.profile) return <PublicPersonalPage data={state.data} />;
  const { business, ownerProfile, listings } = state.data;
  const location = business.location_visibility === 'exact' ? [business.address, business.area, business.city, business.state, business.country].filter(Boolean).join(', ') : [business.area, business.city, business.state, business.country].filter(Boolean).join(', ');
  const hours = getBusinessHoursRows(business.business_hours);
  const todayKey = businessDayLabels[(new Date().getDay() + 6) % 7][0];
  const todayHours = hours.find((item) => item.key === todayKey);
  const whatsappUrl = business.public_contact && business.contact_preference !== 'call' ? normalizeWhatsAppUrl(business.whatsapp) : '';
  const showCall = business.public_contact && business.contact_preference !== 'whatsapp' && Boolean(business.phone);
  const showWhatsApp = Boolean(whatsappUrl);
  const socialLinks = Object.entries(business.social_links || {}).filter(([, value]) => String(value || '').trim());
  const share = async () => { const url = `https://www.bese26.shop/${business.business_handle}`; if (navigator.share) await navigator.share({ title: business.business_name, text: business.description || business.business_name, url }); else { await navigator.clipboard?.writeText(url); setNotice('Store link copied.'); } };
  const toggleBusinessFollow = async () => { if (!viewer) { setNotice('Sign in to follow this business and receive new listing alerts.'); return; } if (viewer.id === business.profile_id) { setNotice('You cannot follow your own business.'); return; } setFollowBusy(true); try { const next = !followState.following; await toggleFollow(viewer.id, business.profile_id, next); setFollowState((current) => ({ ...current, following: next, followers: Math.max(0, current.followers + (next ? 1 : -1)) })); setNotice(next ? 'You are now following this business.' : 'Business removed from your following list.'); } catch (error) { setNotice(error.message || 'Could not update business follow status.'); } finally { setFollowBusy(false); } };
  return <div className="public-business-shell"><header className="public-business-topbar"><a href="https://www.bese26.shop/" className="public-brand"><img className="brand-mark-image" src="/images/bese26-official-logo.png" alt="Bese26" /><strong>Bese26<span>.shop</span></strong></a><div className="public-business-top-actions">{viewer?.id === business.profile_id && <button type="button" className="primary-button" onClick={() => window.location.assign('/?business_dashboard=1')}><Pencil size={15} /> Edit store</button>}<button type="button" className={`secondary-button ${followState.following ? 'is-following' : ''}`} onClick={toggleBusinessFollow} disabled={followBusy || viewer?.id === business.profile_id}><Heart size={15} fill={followState.following ? 'currentColor' : 'none'} /> {followBusy ? 'Saving…' : followState.following ? 'Following' : 'Follow business'}</button><button type="button" className="secondary-button" onClick={share}>Share profile</button></div></header><main className="public-business-main"><section className="miniweb-cover-banner" style={business.logo_path ? { backgroundImage: `linear-gradient(110deg, rgba(18,19,22,.96), rgba(18,19,22,.72)), url(${getAvatarUrl(business.logo_path)})` } : undefined}><div className="miniweb-cover-copy"><span className="eyebrow">WELCOME TO {business.business_name.toUpperCase()}</span><h2>{business.description ? business.description.slice(0, 90) : `Quality products and trusted service from ${business.business_name}.`}</h2><div className="miniweb-cover-pills"><span><ShieldCheck size={14} /> Trusted marketplace</span>{business.delivery_available && <span><ShoppingBasket size={14} /> Delivery</span>}{business.pickup_available && <span><MapPin size={14} /> Pickup</span>}</div></div><div className="miniweb-cover-mark"><Sparkles size={20} /><span>{business.category || 'BUSINESS'}</span></div></section>{notice && <div className="public-business-notice" role="status">{notice}<button type="button" className="icon-button" aria-label="Dismiss notice" onClick={() => setNotice('')}><X size={15} /></button></div>}<section className="public-business-hero"><div className="public-business-logo">{business.logo_path ? <img src={getAvatarUrl(business.logo_path)} alt={`${business.business_name} logo`} /> : <span>{business.business_name.slice(0, 1).toUpperCase()}</span>}</div><div className="public-business-identity"><div className="eyebrow">PUBLIC BUSINESS PROFILE</div><h1>{business.business_name}</h1><strong className="public-business-handle">@{business.business_handle}</strong>{business.is_verified && <span className="verified-badge"><BadgeCheck size={13} /> {business.verification_kind === 'unregistered' ? 'Verified Seller' : 'Verified Business'}</span>}<p>{business.description || 'This business has not added a public description yet.'}</p><span className="public-business-location"><MapPin size={14} /> {location || 'Nigeria'}</span><div className="public-business-actions">{showCall && <a className="primary-button" href={`tel:${business.phone}`}><Phone size={15} /> Call</a>}{showWhatsApp && <a className="secondary-button" href={whatsappUrl} target="_blank" rel="noreferrer"><MessageCircle size={15} /> WhatsApp</a>}{business.website && <a className="secondary-button" href={business.website} target="_blank" rel="noreferrer"><ArrowRight size={15} /> Website</a>}{listings[0] && <a className="secondary-button" href={`/?chat_listing=${listings[0].id}`}><MessageCircle size={15} /> Message</a>}<button type="button" className="secondary-button" onClick={share}>Share</button></div><div className="public-business-follow-meta"><Heart size={14} fill={followState.following ? 'currentColor' : 'none'} /> {followState.followers || 0} follower{followState.followers === 1 ? '' : 's'} · {followState.following ? 'You follow this store' : 'Follow for new listings'}</div></div></section><section className="miniweb-welcome-card"><div className="miniweb-welcome-icon"><Sparkles size={18} /></div><div><div className="eyebrow">A NOTE FROM {business.business_name.toUpperCase()}</div><p>{business.description || `Barka da zuwa ${business.business_name}. Muna maraba da ku ku duba kayayyakinmu kuma ku tuntube mu ta WhatsApp, waya, ko sakon Bese26.`}</p></div></section><section className="miniweb-trust-strip"><span><ShieldCheck size={15} /> {business.is_verified ? 'Verified business' : 'Bese26 seller'}</span><span><Star size={15} /> Fast buyer response</span><span><Package size={15} /> {listings.length} available listing{listings.length === 1 ? '' : 's'}</span><span><Share2 size={15} /> Easy to share</span></section><section className="public-business-info-grid"><div className="public-business-info-card"><div className="eyebrow">OPENING HOURS</div><strong>{todayHours ? `${todayHours.label}: ${todayHours.value}` : 'Hours not added yet'}</strong>{hours.length > 0 && <div className="public-business-hours-list">{hours.map((item) => <span key={item.key}><small>{item.label}</small><b>{item.value}</b></span>)}</div>}</div><div className="public-business-info-card"><div className="eyebrow">SHOP OPTIONS</div><strong>{business.delivery_available || business.pickup_available ? 'Ways to receive your order' : 'Contact the business for arrangements'}</strong><div className="public-business-option-list">{business.delivery_available && <span><Check size={14} /> Delivery available</span>}{business.pickup_available && <span><MapPin size={14} /> Pickup available</span>}{!business.delivery_available && !business.pickup_available && <span><MessageCircle size={14} /> Ask in chat</span>}</div></div>{socialLinks.length > 0 && <div className="public-business-info-card"><div className="eyebrow">CONNECT</div><strong>Follow the business online</strong><div className="public-business-socials">{socialLinks.map(([name, value]) => { const href = String(value).startsWith('http') ? value : `https://${value}`; return <a key={name} href={href} target="_blank" rel="noreferrer">{name.replaceAll('_', ' ')}</a>; })}</div></div>}</section><section className="public-business-owner"><div className="eyebrow">PERSONAL PROFILE LINKED TO THIS BUSINESS</div><div className="public-owner-row">{ownerProfile?.avatar_path ? <img src={getAvatarUrl(ownerProfile.avatar_path)} alt={ownerProfile.display_name || 'Owner'} /> : <div className="public-owner-avatar">{(ownerProfile?.display_name || 'B').slice(0, 1).toUpperCase()}</div>}<div><strong>{ownerProfile?.display_name || 'Bese26 member'}</strong><span>{ownerProfile?.username ? `@${ownerProfile.username}` : 'Personal profile'}</span><p>{ownerProfile?.bio || 'This business is managed through the owner’s Bese26 personal profile.'}</p></div><button type="button" className={`secondary-button ${followState.following ? 'is-following' : ''}`} onClick={toggleBusinessFollow} disabled={followBusy || viewer?.id === business.profile_id}><Heart size={15} fill={followState.following ? 'currentColor' : 'none'} /> {followState.following ? 'Following' : 'Follow store'}</button></div></section><section className="public-business-stats"><div><strong>{listings.length}</strong><span>Listings</span></div><div><strong>{business.category || '—'}</strong><span>Category</span></div><div><strong>{business.business_type || 'Business'}</strong><span>Type</span></div><div><strong>{followState.followers || 0}</strong><span>Followers</span></div></section><section className="public-business-listings"><div className="section-heading"><div><div className="eyebrow">AVAILABLE NOW</div><h2>Listings from {business.business_name}</h2></div><span>{listings.length} listing{listings.length === 1 ? '' : 's'}</span></div><div className="miniweb-category-chips"><span>All products</span>{[...new Set(listings.map((item) => item.category).filter(Boolean))].slice(0, 5).map((category) => <span key={category}>{category}</span>)}</div>{listings.length ? <div className="product-grid">{listings.map((listing, index) => <article className="product-card miniweb-product-card" key={listing.id}><div className="product-image-wrap">{index < 3 && <span className="miniweb-featured-badge"><Sparkles size={12} /> {index === 0 ? 'Featured' : index === 1 ? 'Popular' : 'New'}</span>}{listing.image ? <img src={listing.image} alt={listing.title} className="product-image" /> : <div className="product-image-placeholder"><Package size={26} /></div>}</div><div className="product-info"><div className="product-price">{listing.price}</div><h3>{listing.title}</h3><div className="product-meta"><MapPin size={13} /> {listing.location}</div><div className="product-foot"><span>{listing.condition}</span><span>{listing.posted}</span></div><div className="business-listing-actions"><a className={`secondary-button ${!business.phone ? 'is-disabled' : ''}`} href={business.phone ? `tel:${business.phone}` : undefined} aria-disabled={!business.phone} title={business.phone ? 'Call this business' : 'Phone number not added yet'} onClick={(event) => { event.stopPropagation(); if (!business.phone) event.preventDefault(); }}><Phone size={15} /> Call</a><a className={`secondary-button ${!whatsappUrl ? 'is-disabled' : ''}`} href={whatsappUrl ? `${whatsappUrl}&text=${encodeURIComponent(`Hello, I am interested in ${listing.title}.`)}` : undefined} target={whatsappUrl ? '_blank' : undefined} rel={whatsappUrl ? 'noreferrer' : undefined} aria-disabled={!whatsappUrl} title={whatsappUrl ? 'Chat on WhatsApp' : 'WhatsApp number not added yet'} onClick={(event) => { event.stopPropagation(); if (!whatsappUrl) event.preventDefault(); }}><MessageCircle size={15} /> WhatsApp</a><a className="primary-button business-listing-message" href={`https://www.bese26.shop/?chat_listing=${listing.id}`} onClick={(event) => event.stopPropagation()}><MessageCircle size={15} /> Message</a></div></div></article>)}</div> : <div className="empty-state"><Package size={26} /><h3>No listings yet.</h3><p>This business has not published any approved listings.</p></div>}</section></main><footer className="public-business-footer">Powered by Bese26.shop · Marketplace by Bese26</footer></div>;
}
