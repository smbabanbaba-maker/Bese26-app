...
function SellView({ onDemoAction }) {
  ...
  const publish = () => { onDemoAction('Listing Published Successfully — your listing is ready.'); setForm({ title: '', price: '', description: '', category: 'Phones & Tablets' }); setGenerated(false); };
  return <div className="page-stack sell-page"><div className="page-title-row"><div><div className="eyebrow">TURN CLUTTER INTO VALUE</div><h1>Sell something.</h1></div><span className="seller-chip">Seller tools</span></div>
    <section className="sell-intro"><div className="sell-intro-icon"><Camera size={23} /></div><div><h3>Start with a great photo.</h3><p>Good listings get noticed faster. Add clear photos to help buyers decide.</p></div><button className="icon-button"><ChevronRight size={18} /></button></section>
    <div className="form-section"><div className="form-section-title"><span className="step-number">01</span><div><div className="eyebrow">MAKE IT LOOK GOOD</div><h2>Add photos</h2></div></div><div className="photo-upload-row"><button className="upload-tile"><Plus size={23} /><span>Add photos</span></button><img src="/images/iphone-13-pro.jpg" alt="Listing photo" /><img src="/images/macbook-air.jpg" alt="Listing photo" /></div></div>
    ...
    <button className="publish-button" onClick={publish}>Publish listing <ArrowRight size={17} /></button><p className="marketplace-note">Review your details before publishing to the marketplace.</p>
  </div>;
}
