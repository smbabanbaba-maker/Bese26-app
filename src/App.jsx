...
function NotificationsView({ user, onAuthRequired, onBack, onNotice, onNavigate, onOpenListing }) {
  ...
  const verificationFor = (item) => { const idVerified = isCurrent(item.actor || {}); const cacVerified = isCurrent(item.business || {}); if (idVerified || cacVerified) return <VerificationBadges idVerified={idVerified} cacVerified={cacVerified} compact />; if (!item.actor_id && !item.business) return <span className="notification-verified" aria-label="Bese26 verified"><BadgeCheck size={14} /></span>; return null; };
  const senderName = (item) => item.business?.business_name || item.actor?.display_name || (item.actor?.username ? `@${item.actor.username}` : 'Bese26');
  const senderImage = (item) => item.listing?.image_url || item.business?.logo_url || item.actor?.avatar_url || '/images/bese26-logo-icon.png';
