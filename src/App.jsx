...
export default function App() {
  ...
  return <div className={`app-shell ${isDark ? 'theme-dark' : ''}`}>
    ...
    <nav className="bottom-nav">{navItems.map(({ key, label, icon: Icon }) => <button key={key} className={`${activeNav === key ? 'active' : ''} ${key === 'sell' ? 'sell-nav' : ''}`} onClick={() => navigate(key)}><span className="nav-icon"><Icon size={key === 'sell' ? 22 : 19} strokeWidth={activeNav === key ? 2.4 : 1.9} /></span><span>{label}</span></button>)}</nav>
