...
function NotificationsView({ user, onAuthRequired, onBack, onNotice, onNavigate, onOpenListing }) {
  const [items, setItems] = useState([]); const [filter, setFilter] = useState('all'); const [loading, setLoading] = useState(Boolean(user)); const [entitlement, setEntitlement] = useState(null); const [sellerListings, setSellerListings] = useState([]); const [boosts, setBoosts] = useState([]); const [dismissed, setDismissed] = useState(() => { try { return JSON.parse(localStorage.getItem('bese26:reminders') || '{}'); } catch { return {}; } });
  ...
  const today = new Date().toISOString().slice(0, 10); const unreadCount = items.filter((item) => !item.read_at).length; const visibleItems = filter === 'unread' ? items.filter((item) => !item.read_at) : items;
  ...
  const showSubscriptionReminder = Boolean(entitlement && !entitlement.is_paid && dismissed.subscription !== today); const showBoostReminder = Boolean(sellerListings.length && !boosts.length && dismissed.boost !== today);
  return <div className="page-stack notifications-page notification-center"><header className="notification-center-header"><div><div className="eyebrow">ACCOUNT CENTER</div><h1>Notifications</h1><p>Stay informed about your listings, messages, payments, and account activity.</p></div><div className="notification-header-icon"><Bell size={23} /><span>{unreadCount}</span></div></header><div className="notification-summary-row"><div><strong>{items.length}</strong><span>Total updates</span></div><div><strong>{unreadCount}</strong><span>Unread</span></div><div><strong>{sellerListings.length}</strong><span>Active listings</span></div></div>{(showSubscriptionReminder || showBoostReminder) && <section className="reminder-center"><div className="eyebrow">RECOMMENDED FOR YOU</div>{showSubscriptionReminder && <article className="reminder-card"><span className="reminder-card-icon"><Sparkles size={17} /></span><div><strong>Grow your Bese26 access</strong><p>You are currently on the free plan. Upgrade when you need more listing capacity and seller tools.</p><button type="button" onClick={() => onNavigate?.('subscription')}>View plans</button></div><button type="button" className="reminder-dismiss" onClick={() => dismissReminder('subscription')} aria-label="Dismiss subscription reminder"><X size={15} /></button></article>}{showBoostReminder && <article className="reminder-card"><span className="reminder-card-icon"><ArrowUpRight size={17} /></span><div><strong>Get more eyes on your listing</strong><p>You have active listings without a boost. Promote one when you want more visibility.</p><button type="button" onClick={() => { onNavigate?.('profile'); onNotice?.('Open Profile → Boosting to promote a listing.'); }}>Boost a listing</button></div><button type="button" className="reminder-dismiss" onClick={() => dismissReminder('boost')} aria-label="Dismiss boost reminder"><X size={15} /></button></article>}</section>}<div className="notification-toolbar"><div className="notification-filters" role="tablist" aria-label="Notification filters"><button type="button" className={filter === 'all' ? 'active' : ''} onClick={() => setFilter('all')}>All updates</button><button type="button" className={filter === 'unread' ? 'active' : ''} onClick={() => setFilter('unread')}>Unread <span>{unreadCount}</span></button></div><button type="button" className="notification-refresh" onClick={() => window.location.reload()}>Refresh</button></div>{loading ? <div className="empty-state">Loading notifications…</div> : visibleItems.length ? <div className="notification-list">{visibleItems.map((item) => <button type="button" className={`notification-item ${item.read_at ? '' : 'unread'}`} key={item.id} onClick={() => open(item)}><span className="notification-dot"><Bell size={15} /></span><span><strong>{item.title || 'Bese26 update'}</strong><small>{item.body || item.message || 'You have a new update.'}</small><em>{item.created_at ? new Date(item.created_at).toLocaleString('en-NG', { dateStyle: 'medium', timeStyle: 'short' }) : ''}</em></span>{!item.read_at && <i aria-label="Unread notification" />}</button>)}</div> : <div className="notifications-empty compact"><Bell size={26} /><h2>{filter === 'unread' ? 'You are all caught up' : 'No notifications yet'}</h2><p>{filter === 'unread' ? 'There are no unread updates right now.' : 'New messages, listing updates, and account alerts will appear here.'}</p></div>}</div>;
}
