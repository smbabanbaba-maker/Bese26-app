...
function NotificationsView({ user, onAuthRequired, onBack, onNotice, onNavigate, onOpenListing }) {
  ...
  const verificationFor = (item) => { if (isAdminNotification(item) || (!item.actor_id && !identityBusiness(item))) return <span className="notification-verified notification-admin-verified" aria-label="Bese26 verified"><BadgeCheck size={14} /></span>; const business = identityBusiness(item); const idVerified = !business && isCurrent(item.actor || {}); const cacVerified = Boolean(business && isCurrent(business)); if (idVerified || cacVerified) return <VerificationBadges idVerified={idVerified} cacVerified={cacVerified} compact />; return null; };
