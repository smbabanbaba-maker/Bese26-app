  ...
  return (
    <article className={`product-card ${compact ? 'product-card-compact' : ''}`} role="button" tabIndex={0} aria-label={`Open listing: ${listing.title}`} onClick={() => onOpen(listing)} onKeyDown={(event) => { if ((event.key === 'Enter' || event.key === ' ') && event.target === event.currentTarget) { event.preventDefault(); onOpen(listing); } }}>
      ...
      <div className="product-info">
        ...
        <div className="listing-seller-line"><span className="listing-seller-avatar">{listing.sellerAvatar ? <img src={listing.sellerAvatar} alt="" /> : listing.sellerInitials || 'BE'}</span><div className="listing-seller-copy"><span className="listing-seller-name">{listing.sellerDisplayName || listing.seller}</span><VerificationBadges idVerified={listing.idVerified} cacVerified={listing.cacVerified} compact /></div></div>
...
function NotificationsView({ user, onAuthRequired, onBack, onNotice, onNavigate, onOpenListing }) {
  ...
  const open = async (item) => { if (!item.read_at) { try { await markNotificationRead(item.id, user.id); setItems((current) => current.map((row) => row.id === item.id ? { ...row, read_at: new Date().toISOString() } : row)); } catch {} } const listingId = item.data?.listing_id; if (listingId && onOpenListing) { try { const listing = await fetchListingDetails(listingId); if (listing) onOpenListing(listing); } catch { onNotice?.('This listing is no longer available.'); } } };
  ...
  const senderName = (item) => isAdminNotification(item) ? 'Bese26' : identityBusiness(item)?.business_name || (!item.actor_id ? 'Bese26' : item.actor?.display_name || (item.actor?.username ? `@${item.actor.username}` : 'Bese26'));
  const senderImage = (item) => isAdminNotification(item) && !item.data?.listing_id ? '/images/bese26-logo-icon.png' : item.data?.listing_id && item.listing?.image_url ? item.listing.image_url : identityBusiness(item)?.logo_url || item.actor?.avatar_url || '/images/bese26-logo-icon.png';
  ...
  const renderRow = (item) => { const message = messageFor(item); const isExpanded = Boolean(expanded[item.id]); const isLong = message.length > 150; const displayMessage = !isExpanded && isLong ? `${message.slice(0, 150).trimEnd()}…` : message; return <article className={`notification-feed-row ${item.read_at ? 'is-read' : 'is-unread'}`} key={item.id} role="button" tabIndex={0} onClick={() => open(item)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); open(item); } }}><div className="notification-feed-media"><img src={senderImage(item)} alt="" onError={(event) => { event.currentTarget.src = '/images/bese26-logo-icon.png'; }} /></div><div className="notification-feed-content"><div className="notification-feed-heading"><strong>{senderName(item)}</strong>{verificationFor(item)}</div><p>{displayMessage}</p><div className="notification-feed-meta"><time>{relativeTime(item.created_at)}</time>{isLong && <button type="button" className="notification-more" onClick={(event) => { event.stopPropagation(); setExpanded((current) => ({ ...current, [item.id]: !isExpanded })); }}>{isExpanded ? 'Less' : 'More'}</button>}</div></div>{!item.read_at && <span className="notification-unread-dot" aria-label="Unread" />}</article>; };
...
const subscriptionPlans = [
  ...
  { key: 'basic', name: 'Basic', price: 2500, cadence: 'month', summary: 'For a growing local seller.', highlight: '10 active listings', features: ['Everything in Free', '10 active listings', 'Basic seller analytics', 'Listing management tools', 'Saved searches', 'Standard seller support', 'Personal or business publishing identity', 'Public seller or business link'], tone: 'basic' },
...
function HomeView({ user, marketListings, adCampaigns = [], userPlace = '', locationBusy = false, onUseLocation, onOpenListing, savedIds, onToggleSave, onSearch, onNavigate, onShowNotifications }) {
  ...
  return (
    <div className="page-stack home-page">
      {promo && <section style={promo.image_only ? { aspectRatio: `${promo.creative_width} / ${promo.creative_height}` } : undefined} className={`home-ad-banner ${promo.type === 'dashboard' ? 'home-ad-dashboard dashboard-welcome-card' : `home-ad-slide-${promoIndex}`}`} aria-label={promo.type === 'dashboard' ? 'Your Bese26 dashboard' : 'Sponsored promotion'}>
        {promo.type === 'dashboard' ? <><div className="dashboard-welcome-copy"><div className="eyebrow">{user ? 'YOUR BESE26 DASHBOARD' : 'WELCOME TO BESE26'}</div><h1>{user ? <>Good to see you, <span>{displayName}</span>.</> : <>Buy and sell with confidence.</>}</h1><p>{user ? 'Pick up where you left off and keep your marketplace moving.' : 'Browse trusted listings or start selling in a few simple steps.'}</p><div className="dashboard-actions"><button type="button" className="dashboard-primary-action" onClick={() => onNavigate(user ? 'sell' : 'search')}><Plus size={15} /> {user ? 'List an item' : 'Browse listings'}</button><button type="button" className="dashboard-secondary-action" onClick={() => onNavigate(user ? 'profile' : 'sell')}>{user ? 'View profile' : 'Start selling'} <ArrowRight size={15} /></button></div></div><div className="dashboard-orbit-art" aria-hidden="true"><span className="dashboard-orbit dashboard-orbit-one" /><span className="dashboard-orbit dashboard-orbit-two" /><img src="/images/bese26-logo-icon.png" alt="" /></div></> : promo.image_only ? promo.cta_target?.trim() ? <button type="button" className="home-ad-image-only" onClick={promo.onAction} aria-label={promo.title}><img src={promo.image_url} alt={promo.title} /></button> : <div className="home-ad-image-only" aria-label={promo.title}><img src={promo.image_url} alt={promo.title} /></div> : <><div className="home-ad-copy"><div className="eyebrow light">{promo.eyebrow} <span className="home-ad-sponsored">Sponsored space</span></div><h2>{promo.title}</h2><p>{promo.body}</p><button type="button" className="home-ad-cta" onClick={promo.onAction}>{promo.action} <ArrowRight size={15} /></button></div><div className={`home-ad-art ${promo.type === 'ad' ? 'home-ad-art-clickable' : ''}`} aria-hidden={promo.type !== 'ad'} onClick={promo.type === 'ad' ? promo.onAction : undefined} role={promo.type === 'ad' ? 'link' : undefined} tabIndex={promo.type === 'ad' ? 0 : undefined} onKeyDown={promo.type === 'ad' ? (event) => { if (event.key === 'Enter' || event.key === ' ') promo.onAction(); } : undefined}><img src={promo.image_url || "/images/bese26-official-logo.png"} alt={promo.type === 'ad' ? `${promo.title} advertisement` : ''} /></div></>}
...
function MessagesView({ user, liveListing, onDemoAction, onAuthRequired, onOpenListing, initialMessageId, initialDealPanel = '', initialText = '', onSelectConversation, onBackToInbox }) {
  ...
  const conversationIdentity = (conversation) => { const profile = conversation?.buyer_id === user?.id ? conversation?.seller : conversation?.buyer; const business = profile?.business; return { profile, business, name: business?.business_name || profile?.display_name || 'Bese26 member', image: business?.logo_url || profile?.avatar_url || '' }; };
...
function ListingDetailsView({ listing, user, onClose, isSaved, onToggleSave, onDemoAction, onAuthRequired, onStartChat, onEditListing, onOpenListing }) {
  const [activeImage, setActiveImage] = useState(0);
  ...
  const validImages = [...new Set(galleryCandidates.map((image) => typeof image === 'string' ? image.trim() : image).filter(isUsableGalleryUrl))].filter((image) => !failedImageUrls.has(image));
  const owner = Boolean(user?.id && listing?.sellerId === user.id);
  const raw = listing?.raw || {};
  const description = listing?.description || '';
  const sellerPhone = contact.phone || '';
  const whatsapp = contact.whatsapp || '';
  const phoneEnabled = Boolean(String(sellerPhone).trim());
  ...
  const categoryFields = categoryText.includes('phone') || categoryText.includes('mobile') ? ['brand', 'model', 'condition', 'ram', 'storage', 'colour', 'color'] : categoryText.includes('car') || categoryText.includes('vehicle') ? ['make', 'model', 'year', 'mileage', 'transmission', 'fuel', 'condition'] : categoryText.includes('property') || categoryText.includes('house') || categoryText.includes('land') ? ['property_type', 'bedrooms', 'bathrooms', 'furnishing', 'size', 'condition'] : categoryText.includes('drone') ? ['brand', 'model', 'condition', 'camera', 'flight_type'] : categoryText.includes('electronic') ? ['brand', 'model', 'condition', 'type'] : null;
  const specs = Object.entries(listing?.attributes || {}).filter(([key, value]) => value !== null && value !== undefined && String(value).trim() !== '' && (!categoryFields || categoryFields.includes(String(key).toLowerCase()))).map(([key, value]) => ({ key: key.replaceAll('_', ' ').replace(/\b\w/g, (letter) => letter.toUpperCase()), value: Array.isArray(value) ? value.join(', ') : value }));
  const delivery = Array.isArray(listing?.deliveryOptions) ? listing.deliveryOptions.filter(Boolean) : [];
  const deliveryDetails = delivery.flatMap((item) => { if (typeof item === 'object') return [item.label || item.name || item.type, item.fee != null ? `Delivery fee: ${item.fee}` : '', item.location ? `Pickup: ${item.location}` : '']; return [String(item).replaceAll('_', ' ')]; }).filter(Boolean);
  useEffect(() => { if (!listing) return undefined; setActiveImage(0); setFailedImageUrls(new Set()); setZoomed(false); setExpandedDescription(false); setReviewsOpen(false); setOfferOpen(false); setOfferAmount(''); setReportOpen(false); setContact({ phone: '', whatsapp: '' }); setContactLoading(true); setLoadingDetails(true); setFollowingSeller(false); setSellerListings([]); setSimilarVisibleCount(12); Promise.allSettled([fetchSellerReviews(listing.sellerId), fetchListingReviews(listing.id), fetchSimilarListings(listing), fetchSellerListings(listing), fetchListingContact(listing.id), user?.id && listing.sellerId ? getFollowState(user.id, listing.sellerId) : Promise.resolve({ following: false })]).then(([reviewResult, _listingReviewResult, similarResult, sellerResult, contactResult, followResult]) => { if (reviewResult.status === 'fulfilled') setReviews(reviewResult.value || []); if (similarResult.status === 'fulfilled') setSimilar(similarResult.value || []); if (sellerResult.status === 'fulfilled') setSellerListings(sellerResult.value || []); if (contactResult.status === 'fulfilled') setContact(contactResult.value || { phone: '', whatsapp: '' }); if (followResult.status === 'fulfilled') setFollowingSeller(Boolean(followResult.value?.following)); }).finally(() => { setLoadingDetails(false); setContactLoading(false); }); recordListingView(listing.id).catch(() => {}); return undefined; }, [listing?.id, listing?.sellerId, user?.id]);
  useEffect(() => { const node = similarSentinelRef.current; if (!node || similarVisibleCount >= similar.length) return undefined; const observer = new IntersectionObserver((entries) => { if (entries[0]?.isIntersecting) setSimilarVisibleCount((count) => Math.min(count + 12, similar.length)); }, { rootMargin: '280px' }); observer.observe(node); return () => observer.disconnect(); }, [similar.length, similarVisibleCount]);
  ...
  const normalizePhoneHref = (value) => { const rawValue = String(value || '').trim(); if (!rawValue) return ''; const hasPlus = rawValue.startsWith('+'); const digits = rawValue.replace(/\D/g, ''); return digits ? `tel:${hasPlus ? '+' : ''}${digits}` : ''; };
  const normalizeWhatsAppHref = (value) => { const rawValue = String(value || '').trim(); if (!rawValue) return ''; if (/^https?:\/\//i.test(rawValue)) return rawValue; const digits = rawValue.replace(/\D/g, ''); if (!digits) return ''; const international = digits.startsWith('0') ? `234${digits.slice(1)}` : digits; return `https://wa.me/${international}?text=${encodeURIComponent(`Hello, I am interested in ${listing.title}.`)}`; };
  const callSeller = () => { const href = normalizePhoneHref(sellerPhone); if (!href) { onDemoAction?.(contactLoading ? 'Loading the seller contact details…' : 'The seller has not enabled phone calls for this listing.'); return; } window.location.href = href; };
  const openWhatsApp = () => { const href = normalizeWhatsAppHref(whatsapp); if (!href) { onDemoAction?.(contactLoading ? 'Loading the seller contact details…' : 'The seller has not enabled WhatsApp for this listing.'); return; } window.location.assign(href); };
  const followSeller = async () => { if (!user) { onAuthRequired?.('Sign in to follow sellers and receive new listing alerts.'); return; } if (owner || !listing.sellerId) return; const next = !followingSeller; setFollowingSeller(next); try { await toggleFollow(user.id, listing.sellerId, next); onDemoAction?.(next ? 'You are now following this seller.' : 'You unfollowed this seller.'); } catch (error) { setFollowingSeller(!next); onDemoAction?.(error.message || 'Could not update your following list.'); } };
  const requestCallback = async () => { if (!user) { onAuthRequired?.('Sign in to request a callback from the seller.'); return; } if (owner) { onDemoAction?.('You cannot request a callback from your own listing.'); return; } if (!listing.sellerId) { onDemoAction?.('This listing has no valid seller profile yet.'); return; } setActionBusy(true); try { await requestListingCallback({ listingId: listing.id, requesterId: user.id, sellerId: listing.sellerId, message: `Please call me about ${listing.title}.` }); onDemoAction?.('Callback request sent.'); } catch (error) { onDemoAction?.(error.message || 'Could not request a callback.'); } finally { setActionBusy(false); } };
  const submitListingOffer = async (event) => { event.preventDefault(); if (!user) { onAuthRequired?.('Sign in to make an offer.'); return; } if (owner) { onDemoAction?.('You cannot make an offer on your own listing.'); return; } const amount = Number(offerAmount); if (!Number.isFinite(amount) || amount <= 0) { onDemoAction?.('Enter a valid positive offer amount.'); return; } setOfferBusy(true); try { const conversation = await getOrCreateConversation({ listingId: listing.id, buyerId: user.id, sellerId: listing.sellerId }); await createChatOffer({ conversationId: conversation.id, listingId: listing.id, buyerId: user.id, sellerId: listing.sellerId, amount }); setOfferOpen(false); setOfferAmount(''); onDemoAction?.('Offer sent securely to the seller.'); onStartChat?.(listing, 'message', `I sent an offer of NGN ${amount.toLocaleString('en-NG')} for this listing.`); } catch (error) { onDemoAction?.(error.message || 'Could not send this offer.'); } finally { setOfferBusy(false); } };
  ...
  const sellerSince = raw.profiles?.created_at ? new Date(raw.profiles.created_at).toLocaleDateString('en-NG', { month: 'short', year: 'numeric' }) : '2026';
  const sellerLocation = listing.location || 'Nigeria';
  const primaryImage = validImages[activeImage] || '';
  return <div className="listing-modal-backdrop listing-details-backdrop" role="dialog" aria-modal="true" aria-label="Listing details" onClick={(event) => event.target === event.currentTarget && onClose?.()}>
    <div className="listing-details-view">
      <header className="listing-new-header"><button type="button" className="listing-back-button" onClick={onClose} aria-label="Back"><ArrowLeft size={20} /></button><strong>Listing details</strong><div className="listing-new-header-actions"><button type="button" onClick={() => onToggleSave?.(listing.id)} aria-label={isSaved ? 'Remove from saved' : 'Save listing'}><Heart size={20} fill={isSaved ? 'currentColor' : 'none'} /></button><button type="button" onClick={share} aria-label="Share listing"><Share2 size={19} /></button></div></header>
      <main className="listing-new-content">
        <section className="listing-new-image-section"><div className="listing-new-image-frame" onTouchStart={(event) => setTouchStart(event.changedTouches[0].clientX)} onTouchEnd={(event) => { if (touchStart == null) return; const delta = event.changedTouches[0].clientX - touchStart; if (Math.abs(delta) > 45) delta < 0 ? nextImage() : previousImage(); setTouchStart(null); }}>
          {primaryImage ? <button type="button" className="listing-new-image-button" onClick={() => setZoomed(true)}><img src={primaryImage} alt={`${listing.title} image ${activeImage + 1}`} onError={() => setFailedImageUrls((current) => new Set([...current, primaryImage]))} /></button> : <div className="listing-new-no-image"><ImageIcon size={30} /><span>No listing photo</span></div>}
          {validImages.length > 1 && <><button type="button" className="listing-new-gallery-arrow prev" onClick={previousImage} aria-label="Previous photo"><ArrowLeft size={17} /></button><button type="button" className="listing-new-gallery-arrow next" onClick={nextImage} aria-label="Next photo"><ArrowRight size={17} /></button></>}
          <span className="listing-new-counter">{validImages.length ? `${activeImage + 1}/${validImages.length}` : '0/0'}</span>
        </div>{validImages.length > 1 && <div className="listing-new-thumbs">{validImages.map((image, index) => <button type="button" key={`${image}-${index}`} className={activeImage === index ? 'active' : ''} onClick={() => setActiveImage(index)} aria-label={`View photo ${index + 1}`}><img src={image} alt="" /></button>)}</div>}</section>
        <section className="listing-new-section listing-new-summary"><div className="listing-new-meta"><MapPin size={15} /> {listing.location || 'Nigeria'} <span>·</span> {listing.posted || 'Recently'}</div><h1>{listing.title}</h1><div className="listing-new-price">{listing.price}</div><span className="listing-new-condition">{listing.condition}</span></section>
        <section className="listing-new-section listing-new-contact"><div className="listing-new-action-grid">{phoneEnabled && <><button type="button" className="listing-new-outline-action" onClick={requestCallback} disabled={actionBusy}><Phone size={16} /> Request callback</button><button type="button" className="listing-new-solid-action" onClick={callSeller}><Phone size={16} /> Call</button></>}{!phoneEnabled && !whatsappEnabled && <p className="listing-contact-note">Seller prefers private messages for this listing.</p>}</div>{whatsappEnabled && <button type="button" className="listing-new-whatsapp" onClick={openWhatsApp}><MessageCircle size={15} /> WhatsApp seller</button>} {!owner && <button type="button" className="listing-new-message-action" onClick={() => onStartChat?.(listing, 'message')}><MessageCircle size={15} /> Message seller</button>}</section>
        <section className="listing-new-section listing-new-chat"><div className="listing-new-section-heading"><div><span className="listing-new-kicker">PRIVATE CONVERSATION</span><h2>Chat with the seller</h2></div><MessageCircle size={20} /></div><div className="listing-new-quick-actions"><button type="button" onClick={() => setQuickMessage('Is this still available?')}>Is this available?</button><button type="button" onClick={() => setQuickMessage('What is your final price?')}>Last price</button>{!owner && <button type="button" onClick={() => user ? setOfferOpen(true) : onAuthRequired?.('Sign in to make an offer.')}>Make an offer</button>}</div><textarea value={quickMessage} onChange={(event) => setQuickMessage(event.target.value)} placeholder="Write your message here…" maxLength={500} /><button type="button" className="listing-new-start-chat" onClick={() => onStartChat?.(listing, 'message', quickMessage)}>Start chat <ArrowRight size={16} /></button></section>
        ...
        <section className="listing-new-section listing-new-seller"><div className="listing-new-seller-head"><div className="listing-new-seller-avatar">{listing.sellerAvatar ? <img src={listing.sellerAvatar} alt="" /> : listing.sellerInitials}</div><div><h2>{listing.sellerDisplayName || listing.seller}</h2><VerificationBadges idVerified={listing.idVerified} cacVerified={listing.cacVerified} compact /><p><MapPin size={13} /> {sellerLocation} · Member since {sellerSince}</p></div><button type="button" className="listing-new-link" onClick={() => listing.sellerBusinessHandle ? window.location.assign(`/@${listing.sellerBusinessHandle}`) : listing.sellerId && raw.profiles?.username ? window.location.assign(`/${raw.profiles.username}`) : onDemoAction?.('Seller profile is not public yet.')}>View store <ChevronRight size={15} /></button></div><div className="listing-new-seller-actions"><button type="button" onClick={followSeller} disabled={owner || !listing.sellerId}><UserPlus size={15} /> {followingSeller ? 'Following' : 'Follow'}</button><button type="button" onClick={share}><Share2 size={15} /> Share</button></div></section>
        <section className="listing-new-section listing-new-reviews"><div className="listing-new-section-heading"><div><h2>Feedback about seller</h2>{reviews.length > 0 && <span className="listing-review-summary">★ {(reviews.reduce((sum, review) => sum + Number(review.rating || 0), 0) / reviews.length).toFixed(1)} · {reviews.length} reviews</span>}</div>{reviews.length > 2 && <button type="button" className="listing-new-link" onClick={() => setReviewsOpen(true)}>View all <ChevronRight size={14} /></button>}</div>{reviews.length ? reviews.slice(0, 2).map((review) => <article className="listing-new-review" key={review.id}><div className="listing-new-review-top"><div className="listing-new-review-avatar">{(review.reviewer?.display_name || 'B').slice(0, 1).toUpperCase()}</div><strong>{review.reviewer?.display_name || 'Buyer'}</strong><time>{new Date(review.created_at).toLocaleDateString('en-NG', { day: 'numeric', month: 'short' })}</time></div><div className="listing-new-stars">{'★'.repeat(Math.max(0, Math.min(5, Number(review.rating) || 0)))}</div><p>{review.body || 'No written review.'}</p>{review.listing?.title && <small className="listing-review-listing">About: {review.listing.title}</small>}</article>) : <div className="listing-new-no-reviews"><span>☆</span><p>No reviews yet</p></div>}</section>
        <section className="listing-new-safety"><div><ShieldCheck size={20} /><div><strong>Stay safe</strong><p>Meet in a public place, inspect the item before paying, and never share OTPs, passwords or PINs.</p></div></div><button type="button" onClick={() => setReportOpen(true)}>Report listing</button></section>
        {sellerListings.length > 0 && <section className="listing-new-section listing-new-similar"><div className="listing-new-section-heading"><h2>More from {listing.sellerDisplayName || listing.seller}</h2><button type="button" className="listing-new-link" onClick={() => listing.sellerBusinessHandle ? window.location.assign(`/@${listing.sellerBusinessHandle}`) : raw.profiles?.username ? window.location.assign(`/${raw.profiles.username}`) : null}>View store <ChevronRight size={15} /></button></div><div className="listing-new-feed">{sellerListings.map((item) => <button type="button" className="listing-new-card" key={item.id} onClick={() => onOpenListing?.(item)}><div className="listing-new-card-image">{item.image ? <img src={item.image} alt={item.title} loading="lazy" /> : <Package size={22} />}<span aria-label="Save listing"><Heart size={15} /></span></div><strong>{item.title}</strong><b>{item.price}</b><small><MapPin size={11} /> {item.location}</small></button>)}</div></section>}
        {similar.length > 0 && <section className="listing-new-section listing-new-similar"><div className="listing-new-section-heading"><h2>Similar listings</h2><span>{similar.length} available</span></div><div className="listing-new-feed">{similar.filter((item, index, items) => item.id !== listing.id && items.findIndex((candidate) => candidate.id === item.id) === index).slice(0, similarVisibleCount).map((item) => <button type="button" className="listing-new-card" key={item.id} onClick={() => onOpenListing?.(item)}><div className="listing-new-card-image">{item.image ? <img src={item.image} alt={item.title} loading="lazy" /> : <Package size={22} />}<span aria-label="Save listing"><Heart size={15} /></span></div><strong>{item.title}</strong><b>{item.price}</b><small><MapPin size={11} /> {item.location}</small></button>)}</div>{similarVisibleCount < similar.length && <div ref={similarSentinelRef} className="listing-new-feed-sentinel" aria-hidden="true">Loading more listings…</div>}</section>}
        {owner && <section className="listing-owner-controls"><div><strong>Manage your listing</strong><span>{Number(raw.views_count || 0)} real views · status: {raw.status || 'active'}</span></div><div><button type="button" onClick={() => onEditListing?.(listing)}>Edit listing</button><button type="button" disabled={actionBusy} onClick={() => manage(raw.status === 'paused' ? 'active' : 'paused')}>{raw.status === 'paused' ? 'Resume' : 'Pause'}</button><button type="button" disabled={actionBusy} onClick={() => manage('sold')}>Mark sold</button><button type="button" disabled={actionBusy} onClick={remove}>Delete</button></div></section>}
      </main>
    </div>
    ...
    {offerOpen && <div className="listing-action-overlay" role="dialog" aria-modal="true" aria-label="Make an offer" onClick={(event) => event.target === event.currentTarget && setOfferOpen(false)}><form className="listing-action-sheet" onSubmit={submitListingOffer}><div className="listing-action-sheet-head"><div><span className="listing-new-kicker">NEGOTIATE SAFELY</span><h2>Make an offer</h2></div><button type="button" className="icon-button" onClick={() => setOfferOpen(false)} aria-label="Close offer"><X size={18} /></button></div><p>Listing price: <strong>{listing.price}</strong></p><label>Your offer (NGN)<input type="number" min="1" step="1" value={offerAmount} onChange={(event) => setOfferAmount(event.target.value)} placeholder="Enter amount" autoFocus /></label><small>Your offer will be sent in the private conversation. An accepted offer is not a payment.</small><button className="primary-button" disabled={offerBusy}>{offerBusy ? 'Sending offer…' : 'Send offer'}</button></form></div>}
    {reviewsOpen && <div className="listing-action-overlay" role="dialog" aria-modal="true" aria-label="Seller reviews" onClick={(event) => event.target === event.currentTarget && setReviewsOpen(false)}><section className="listing-action-sheet listing-reviews-sheet"><div className="listing-action-sheet-head"><div><span className="listing-new-kicker">SELLER FEEDBACK</span><h2>All reviews</h2></div><button type="button" className="icon-button" onClick={() => setReviewsOpen(false)} aria-label="Close reviews"><X size={18} /></button></div><div className="listing-reviews-list">{reviews.map((review) => <article className="listing-new-review" key={review.id}><div className="listing-new-review-top"><div className="listing-new-review-avatar">{(review.reviewer?.display_name || 'B').slice(0, 1).toUpperCase()}</div><strong>{review.reviewer?.display_name || 'Buyer'}</strong><time>{new Date(review.created_at).toLocaleDateString('en-NG', { day: 'numeric', month: 'short', year: 'numeric' })}</time></div><div className="listing-new-stars">{'★'.repeat(Math.max(0, Math.min(5, Number(review.rating) || 0)))}</div><p>{review.body || 'No written review.'}</p></article>)}</div></section></div>}
  </div>;
}
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (event === 'SIGNED_OUT') { setSavedIds([]); setIsAdmin(false); setSelectedListing(null); setChatListing(null); setChatTargetId(null); setChatDealPanel(''); setChatDraft(''); setEditingListing(null); setSearch(''); setActiveNav('home'); loadBackend(); }
    });
