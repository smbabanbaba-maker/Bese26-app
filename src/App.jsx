...
function MessagesView({ user, liveListing, onDemoAction, onAuthRequired, initialMessageId, initialDealPanel = '', initialText = '', onSelectConversation, onBackToInbox }) {
...
function ListingModal({ listing, user, adCampaigns = [], onClose, isSaved, onToggleSave, onDemoAction, onAuthRequired, onStartChat, onEditListing, onOpenListing }) {
  ...
  const [detailAdIndex, setDetailAdIndex] = useState(0);
...
function AppContent() {
  ...
  return <div className={`app-shell ${isDark ? 'theme-dark' : ''}`}>
    ...
    <ListingModal listing={selectedListing} user={sessionUser} adCampaigns={adCampaigns} onClose={() => setSelectedListing(null)} onAuthRequired={requireAuth} isSaved={selectedListing ? savedIds.includes(selectedListing.id) : false} onToggleSave={toggleSave} onDemoAction={showToast} onStartChat={openChat} onOpenListing={openListing} onEditListing={(item) => { setSelectedListing(null); setEditingListing(item); navigate('sell'); }} />
