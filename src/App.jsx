...
function NotificationsView({ user, onAuthRequired, onBack, onNotice, onNavigate, onOpenListing }) {
  ...
  const isVerified = (item) => !item.actor_id || Boolean(item.actor?.is_verified || item.business?.is_verified || String(item.business?.verification_status || '').toLowerCase() === 'verified');
  ...
  const renderRow = (item) => { const message = messageFor(item); const isExpanded = Boolean(expanded[item.id]); const isLong = message.length > 150; const displayMessage = !isExpanded && isLong ? `${message.slice(0, 150).trimEnd()}…` : message; return <article className={`notification-feed-row ${item.read_at ? 'is-read' : 'is-unread'}`} key={item.id} role="button" tabIndex={0} onClick={() => open(item)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); open(item); } }}><div className="notification-feed-media"><img src={senderImage(item)} alt="" onError={(event) => { event.currentTarget.src = '/images/bese26-logo-icon.png'; }} /></div><div className="notification-feed-content"><div className="notification-feed-heading"><strong>{senderName(item)}</strong>{isVerified(item) && <span className="notification-verified" aria-label="Verified"><BadgeCheck size={14} /></span>}</div><p>{displayMessage}</p><div className="notification-feed-meta"><time>{relativeTime(item.created_at)}</time>{isLong && <button type="button" className="notification-more" onClick={(event) => { event.stopPropagation(); setExpanded((current) => ({ ...current, [item.id]: !isExpanded })); }}>{isExpanded ? 'Less' : 'More'}</button>}</div></div>{!item.read_at && <span className="notification-unread-dot" aria-label="Unread" />}</article>; };
