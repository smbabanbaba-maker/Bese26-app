  ...
  return <div className="listing-modal-backdrop listing-details-backdrop" onClick={(event) => event.target === event.currentTarget && onClose?.()}>
    <div className="listing-details-premium listing-details-page" role="dialog" aria-modal="true" aria-label={`${listing.title} details`}>
      ...
      <main className="listing-details-scroll">
        ...
        <section className="listing-detail-card listing-seller-card"><div className="listing-card-heading"><div><div className="eyebrow">SELLER</div><h2>About the seller</h2></div><button type="button" className="listing-follow-button" onClick={followSeller}>{followingSeller ? 'Following' : 'Follow'}</button></div><div className="listing-seller-profile-row"><span className="listing-seller-avatar listing-seller-avatar-large">{sellerInitial}</span><div><strong>{sellerName}</strong><span>{listing.verified ? <><BadgeCheck size={14} /> Verified seller</> : trustLabel}</span><small><MapPin size={12} /> {sellerLocation || 'Nigeria'}</small></div></div>{(sellerPhone || whatsapp) && <div className="listing-contact-availability"><span>Contact available</span><small>{sellerPhone ? 'Phone enabled' : 'WhatsApp enabled'}</small></div>}</section>
        <section className="listing-detail-card"><div className="listing-card-heading"><div><div className="eyebrow">TRUST</div><h2>Feedback about seller</h2></div><span className="listing-review-count">{reviews.length} review{reviews.length === 1 ? '' : 's'}</span></div>{reviews.length ? reviews.slice(0, 2).map((review) => <article className="review-row review-row-clean" key={review.id}><strong>{review.reviewer?.display_name || 'Buyer'}</strong><span>{'★'.repeat(Math.max(0, Math.min(5, Number(review.rating) || 0)))} · {new Date(review.created_at).toLocaleDateString('en-NG')}</span><p>{review.body || 'No written review.'}</p></article>) : <div className="listing-empty-feedback"><Star size={19} /><span>No reviews yet</span><small>Feedback will appear here after buyers share their experience.</small></div>}</section>
        ...
        {similar.length > 0 && <section className="listing-detail-card"><div className="listing-card-heading"><div><div className="eyebrow">KEEP BROWSING</div><h2>Similar listings</h2></div></div><div className="listing-similar-grid listing-similar-grid-clean">{similar.slice(0, 6).map((item) => <button key={item.id} type="button" onClick={() => onOpenListing?.(item)}><strong>{item.title}</strong><span>{item.price}</span><small>{item.location}</small></button>)}</div></section>}
