...
function Avatar({ initials, path, alt = 'Profile photo', tone = 'rose', size = 'md' }) {
  if (path) {
    const src = /^https?:\/\//i.test(path) ? path : getAvatarUrl(path);
    return <img className={`avatar avatar-${size} profile-avatar-image`} src={src} alt={alt} onError={(event) => { event.currentTarget.replaceWith(Object.assign(document.createElement('div'), { className: `avatar avatar-${tone} avatar-${size}`, textContent: initials || 'BE' })); }} />;
  }
  ...
  return (
    <article className={`product-card ${compact ? 'product-card-compact' : ''}`} role="button" tabIndex={0} aria-label={`Open listing: ${listing.title}`} onClick={() => onOpen(listing)} onKeyDown={(event) => { if ((event.key === 'Enter' || event.key === ' ') && event.target === event.currentTarget) { event.preventDefault(); onOpen(listing); } }}>
      ...
      <div className="product-info">
        ...
        <div className="listing-seller-line">{listing.sellerAvatar ? <img className="listing-seller-avatar listing-seller-avatar-image" src={listing.sellerAvatar} alt={`${listing.sellerDisplayName || listing.seller || 'Seller'} profile`} loading="lazy" onError={(event) => { event.currentTarget.hidden = true; event.currentTarget.nextElementSibling?.removeAttribute('hidden'); }} /> : null}<span className="listing-seller-avatar listing-seller-avatar-fallback" hidden={Boolean(listing.sellerAvatar)}>{listing.sellerInitials || 'BE'}</span><span className="listing-seller-name">{listing.sellerDisplayName || listing.seller}{listing.verified && <BadgeCheck className="unified-verified-icon" size={18} strokeWidth={3} aria-label="Verified account" />}</span></div>
...
function ListingModal({ listing, user, onClose, isSaved, onToggleSave, onDemoAction, onAuthRequired, onStartChat, onEditListing, onOpenListing }) {
  ...
  return <div className="listing-modal-backdrop" role="dialog" aria-modal="true" aria-label="Listing details" onClick={(event) => event.target === event.currentTarget && onClose?.()}><div className="listing-modal listing-details-premium"><div className="listing-modal-header"><span className="eyebrow">LISTING DETAILS</span><div className="listing-modal-header-actions"><button className="icon-button" onClick={() => onToggleSave?.(listing.id)} aria-label={isSaved ? 'Remove from saved' : 'Save listing'}><Heart size={19} fill={isSaved ? 'currentColor' : 'none'} /></button><button className="icon-button" onClick={onClose} aria-label="Close listing details"><X size={19} /></button></div></div><div className="listing-details-layout"><section className="listing-gallery-panel"><div className="listing-gallery-frame" onTouchStart={(event) => setTouchStart(event.changedTouches[0].clientX)} onTouchEnd={(event) => { if (touchStart == null) return; const delta = event.changedTouches[0].clientX - touchStart; if (Math.abs(delta) > 45) delta < 0 ? nextImage() : previousImage(); setTouchStart(null); }}><button className="gallery-main-button" onClick={() => gallery.length && setZoomed(true)} aria-label="Open fullscreen gallery">{gallery.length ? <img src={gallery[activeImage]} alt={`${listing.title} image ${activeImage + 1}`} onError={(event) => { event.currentTarget.style.display = 'none'; event.currentTarget.parentElement?.querySelector('.listing-gallery-fallback')?.removeAttribute('hidden'); }} /> : <div className="listing-gallery-placeholder"><ImageIcon size={38} /><span>No photos available</span></div>}<div className="listing-gallery-fallback" hidden><ImageIcon size={38} /><span>Photo unavailable</span></div></button>{gallery.length > 1 && <><button className="gallery-control gallery-control-prev" onClick={previousImage} aria-label="Previous listing photo"><ArrowLeft size={18} /></button><button className="gallery-control gallery-control-next" onClick={nextImage} aria-label="Next listing photo"><ArrowRight size={18} /></button></>}<span className="modal-image-count">{gallery.length ? `${activeImage + 1} / ${gallery.length}` : 'No image'}</span><button className="image-zoom-button" onClick={() => gallery.length && setZoomed(true)} aria-label="View fullscreen"><ZoomIn size={17} /></button></div>{gallery.length > 1 && <div className="modal-gallery-thumbs">{gallery.map((image, index) => <button key={`${image}-${index}`} className={activeImage === index ? 'active' : ''} onClick={() => setActiveImage(index)} aria-label={`View listing photo ${index + 1}`}><img loading={index > 2 ? 'lazy' : undefined} src={image} alt="" /></button>)}</div>}</section><section className="listing-detail-body"><div className="listing-safety-banner"><ShieldCheck size={17} /><span><strong>Stay safe:</strong> never share OTPs, passwords, or full payment before confirming the seller and deal details.</span></div><div className="listing-location-line"><MapPin size={16} /> {listing.location}{listing.posted && <><span>·</span>{listing.posted}</>}{raw.updated_at && <span>· Updated {new Date(raw.updated_at).toLocaleDateString('en-NG')}</span>}</div><h1>{listing.title}</h1><div className="listing-price-row"><div className="listing-detail-price">{listing.price}</div><span className="listing-pricing-type">{priceType}</span></div><div className="listing-detail-meta"><span>{listing.condition}</span>{titleParts && <span>{titleParts}</span>}<span>{Number(raw.views_count || 0)} views</span></div><section className="trust-summary-card"><div className="trust-summary-heading"><div className="trust-summary-icon"><ShieldCheck size={19} /></div><div><strong>{trustLabel}</strong><span>{trustNote}</span></div></div><div className="trust-summary-grid"><span><b>{listing.sellerRating > 0 ? `${listing.sellerRating.toFixed(1)}/5` : "New"}</b> seller rating</span><span><b>{listing.location || "Local"}</b> meeting area</span><span><b>{Number(raw.views_count || 0)}</b> real views</span></div></section><div className="listing-primary-actions"><button className="callback-contact-button" onClick={requestCallback} disabled={actionBusy}><Phone size={18} /> Request callback</button><button className="primary-contact-button" onClick={callSeller} disabled={contactLoading || !sellerPhone}><Phone size={18} /> {contactLoading ? 'Loading…' : 'Call'}</button><button className="secondary-contact-button" onClick={openWhatsApp} disabled={contactLoading || !whatsapp}><MessageCircle size={18} /> {contactLoading ? 'Loading…' : 'WhatsApp'}</button><button className="secondary-contact-button" onClick={() => onStartChat?.(listing, 'message')}><MessageCircle size={18} /> Message Seller</button><button className="secondary-contact-button" onClick={() => onStartChat?.(listing, 'offer')}><Tag size={18} /> Make an offer</button></div><section className="listing-inline-chat"><div><div className="eyebrow">CHAT WITH THE SELLER</div><h2>Ask about this listing</h2></div><div className="listing-question-pills"><button type="button" onClick={() => setQuickMessage('Is this still available?')}>Is this available?</button><button type="button" onClick={() => setQuickMessage('What is your final price?')}>Last price</button><button type="button" onClick={() => setQuickMessage('Can I pick it up today?')}>Pickup today</button></div><textarea value={quickMessage} onChange={(event) => setQuickMessage(event.target.value)} placeholder="Write your message here" maxLength={500} /><button type="button" className="listing-start-chat" onClick={() => onStartChat?.(listing, 'message', quickMessage)}>Start chat <ArrowRight size={16} /></button></section><section className="listing-question-card"><div><div className="eyebrow">NOT SURE WHAT TO ASK?</div><h2>Start with a simple question</h2><p>Open a protected Bese26 conversation and ask the seller directly.</p></div><div className="listing-question-pills">{buyerQuestions.map((question) => <button type="button" key={question} onClick={() => onStartChat?.(listing, 'message', question)}>{question}</button>)}</div></section><section className="safe-deal-card"><div><ShieldCheck size={18} /><div><strong>Safe Deal checklist</strong><p>Keep payment and questions in Bese26 chat. Meet in a busy public place and inspect the item before paying.</p></div></div><button type="button" onClick={() => onStartChat?.(listing)}>Start a safe deal <ArrowRight size={16} /></button></section><section className="listing-info-card"><div className="listing-section-heading"><h2>Listing details</h2><span>{loadingDetails ? 'Loading…' : `${specs.length} details`}</span></div>{specs.length ? <div className="listing-spec-grid">{specs.map((item) => <div key={item.key}><span>{item.key}</span><strong>{String(item.value)}</strong></div>)}</div> : <p>No additional category attributes were provided.</p>}</section><section className="listing-info-card"><h2>Description</h2><p className={`listing-description listing-description-card ${!expandedDescription && description.length > 500 ? 'is-collapsed' : ''}`}>{description || 'The seller has not added a description yet.'}</p>{description.length > 500 && <button className="text-action" onClick={() => setExpandedDescription((value) => !value)}>{expandedDescription ? 'Show less' : 'Read more'}</button>}</section>{delivery.length > 0 && <section className="listing-info-card"><h2>Delivery</h2><div className="listing-delivery-list">{delivery.map((item) => <span key={String(item)}>{String(item).replaceAll('_', ' ')}</span>)}</div></section>}<section className="seller-profile-card"><div className="seller-profile-heading">{listing.sellerAvatar ? <img className="seller-card-avatar" src={listing.sellerAvatar} alt={`${listing.sellerDisplayName || listing.seller || 'Seller'} profile`} onError={(event) => { event.currentTarget.hidden = true; event.currentTarget.nextElementSibling?.removeAttribute('hidden'); }} /> : null}<span className="seller-card-avatar seller-card-avatar-fallback" hidden={Boolean(listing.sellerAvatar)}>{listing.sellerInitials || 'BE'}</span><div><strong className="seller-name-with-badge">{listing.sellerDisplayName || listing.seller}{listing.verified && <BadgeCheck className="unified-verified-icon" size={19} strokeWidth={3} aria-label="Verified account" />}</strong><span>{listing.sellerBusinessName ? <><Store size={14} /> {listing.sellerBusinessName} · Business</> : 'Personal seller'} {listing.verified && <span className="verified-badge"><BadgeCheck size={12} /> Verified</span>}</span><small>{listing.sellerRating > 0 ? `${listing.sellerRating.toFixed(1)} rating` : 'No rating yet'} · Member since {raw.profiles?.created_at ? new Date(raw.profiles.created_at).toLocaleDateString('en-NG', { month: 'short', year: 'numeric' }) : 'Bese26 member'}</small></div></div><div className="listing-detail-actions"><button onClick={() => listing.sellerBusinessHandle ? window.location.assign(`/@${listing.sellerBusinessHandle}`) : listing.sellerId && raw.profiles?.username ? window.location.assign(`/${raw.profiles.username}`) : onDemoAction?.('Seller profile is not public yet.')}>View profile <ChevronRight size={17} /></button><button onClick={followSeller} disabled={owner || !listing.sellerId}><UserPlus size={16} /> {followingSeller ? 'Following' : 'Follow seller'}</button>{listing.sellerBusinessHandle && <button onClick={() => window.location.assign(`/@${listing.sellerBusinessHandle}`)}><Store size={16} /> View store</button>}<button onClick={share}><Share2 size={16} /> Share</button><button className="report-action" onClick={() => setReportOpen(true)}>Report</button></div></section><section className="listing-info-card"><div className="listing-section-heading"><h2>Feedback about seller</h2><span>{reviews.length} review{reviews.length === 1 ? '' : 's'}</span></div>{reviews.length ? reviews.map((review) => <div className="review-row" key={review.id}><strong>{review.reviewer?.display_name || 'Buyer'}</strong><span>{'★'.repeat(Math.max(0, Math.min(5, Number(review.rating) || 0)))} · {new Date(review.created_at).toLocaleDateString('en-NG')}</span><p>{review.body || 'No written review.'}</p></div>) : <p>No published reviews yet.</p>}</section>{similar.length > 0 && <section className="listing-info-card listing-recommendations-card"><div className="listing-recommendations-heading"><div><div className="eyebrow">KEEP BROWSING</div><h2>More listings like this</h2></div><span>{similar.length} available</span></div><div className="listing-similar-grid">{similar.map((item) => <button key={item.id} onClick={() => onOpenListing?.(item)}><div className="similar-listing-image-wrap">{item.promoted && <span className="similar-boosted-badge"><Sparkles size={10} /> Boosted</span>}{item.image ? <img src={item.image} alt={item.title} loading="lazy" decoding="async" /> : <div className="similar-listing-placeholder"><Package size={24} /></div>}</div><div className="similar-listing-copy"><strong>{item.title}</strong><span>{item.price}</span><small><MapPin size={11} /> {item.location}</small></div></button>)}</div></section>}{owner && <section className="listing-owner-controls"><div><strong>Manage your listing</strong><span>{Number(raw.views_count || 0)} real views · status: {raw.status || 'active'}</span></div><div><button onClick={() => onEditListing?.(listing)}>Edit listing</button><button disabled={actionBusy} onClick={() => manage(raw.status === 'paused' ? 'active' : 'paused')}>{raw.status === 'paused' ? 'Resume' : 'Pause'}</button><button disabled={actionBusy} onClick={() => manage('sold')}>Mark sold</button><button disabled={actionBusy} onClick={remove}>Delete</button></div></section>}</section><div className="listing-mobile-sticky-actions"><div className="listing-mobile-price"><strong>{listing.price}</strong><small>{listing.location}</small></div><button type="button" className="listing-mobile-save" onClick={() => onToggleSave?.(listing.id)} aria-label={isSaved ? "Remove from saved" : "Save listing"}><Heart size={17} fill={isSaved ? "currentColor" : "none"} /></button>{sellerPhone && <button type="button" className="listing-mobile-call" onClick={callSeller} aria-label="Call seller"><Phone size={16} /> Call</button>}{whatsapp && <button type="button" className="listing-mobile-whatsapp" onClick={openWhatsApp} aria-label="WhatsApp seller"><MessageCircle size={16} /> WhatsApp</button>}<button type="button" className="listing-mobile-chat" onClick={() => onStartChat?.(listing, "message")}><MessageCircle size={16} /> Chat</button></div></div></div>{zoomed && <div className="listing-gallery-lightbox" role="dialog" aria-label="Fullscreen listing gallery" onClick={() => setZoomed(false)}><button className="icon-button lightbox-close" onClick={() => setZoomed(false)} aria-label="Close fullscreen"><X size={20} /></button>{gallery.length > 1 && <button className="gallery-control gallery-control-prev" onClick={(event) => { event.stopPropagation(); previousImage(); }} aria-label="Previous photo"><ArrowLeft size={20} /></button>}<img src={gallery[activeImage]} alt={`${listing.title} fullscreen image ${activeImage + 1}`} onClick={(event) => event.stopPropagation()} />{gallery.length > 1 && <button className="gallery-control gallery-control-next" onClick={(event) => { event.stopPropagation(); nextImage(); }} aria-label="Next photo"><ArrowRight size={20} /></button>}</div>}{reportOpen && <div className="listing-report-overlay" role="dialog" aria-label="Report listing" onClick={(event) => event.target === event.currentTarget && setReportOpen(false)}><form className="listing-report-card" onSubmit={submitReport}><button type="button" className="icon-button" onClick={() => setReportOpen(false)} aria-label="Close report"><X size={18} /></button><h2>Report listing</h2><label>Reason<select value={reportReason} onChange={(event) => setReportReason(event.target.value)}>{['scam', 'fake_product', 'counterfeit', 'prohibited_item', 'wrong_information', 'fake_identity', 'harassment', 'suspicious_activity', 'other'].map((reason) => <option key={reason} value={reason}>{reason.replaceAll('_', ' ')}</option>)}</select></label><label>Details<textarea value={reportDetails} onChange={(event) => setReportDetails(event.target.value)} maxLength={1500} placeholder="Tell Bese26 moderation what happened" /></label><button className="primary-button" disabled={actionBusy}>{actionBusy ? 'Submitting…' : 'Submit report'}</button></form></div>}</div>;
}
