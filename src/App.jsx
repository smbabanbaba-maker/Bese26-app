...
function ListingModal({ listing, user, onClose, isSaved, onToggleSave, onDemoAction, onAuthRequired, onStartChat, onEditListing, onOpenListing }) {
  ...
  const sellerLocation = [raw.seller_area, raw.seller_city, raw.seller_state, raw.seller_country].filter(Boolean).join(', ') || listing.location;
  const identityVerified = raw.profiles?.is_verified === true || raw.profiles?.is_verified === 'true' || raw.profiles?.is_verified === 1 || raw.profiles?.is_verified === '1';
  const cacVerified = raw.business_profile?.is_verified === true || raw.business_profile?.is_verified === 'true' || raw.business_profile?.is_verified === 1 || raw.business_profile?.is_verified === '1';
  const descriptionPreview = description.length > 260 && !expandedDescription ? `${description.slice(0, 260).trim()}…` : description;
  return <div className="listing-modal-backdrop listing-details-backdrop" onClick={(event) => event.target === event.currentTarget && onClose?.()}>
    <div className="listing-details-premium listing-details-page" role="dialog" aria-modal="true" aria-label={`${listing.title} details`}>
      ...
      <main className="listing-details-scroll">
        ...
        <section className="listing-detail-card listing-description-card"><div className="listing-card-heading"><div><div className="eyebrow">ABOUT THIS ITEM</div><h2>Description</h2></div></div><p>{descriptionPreview || 'The seller has not added a description yet.'}</p>{description.length > 260 && <button type="button" className="listing-text-toggle" onClick={() => setExpandedDescription((value) => !value)}>{expandedDescription ? 'Show less ↑' : 'Show more ↓'}</button>}</section>
        <section className="listing-detail-card listing-seller-card"><div className="listing-card-heading"><div><h2>Seller</h2></div><button type="button" className="listing-follow-button" onClick={followSeller}>{followingSeller ? 'Following' : 'Follow'}</button></div><div className="listing-seller-profile-row"><span className="listing-seller-avatar listing-seller-avatar-large">{sellerInitial}</span><div><strong>{sellerName}</strong><small><MapPin size={12} /> {sellerLocation || 'Nigeria'}</small><small>Member on Bese26</small></div></div><div className="listing-verification-badges">{identityVerified && <span className="verification-id-badge"><BadgeCheck size={13} /> Verified ID</span>}{cacVerified && <span className="verification-cac-badge"><BadgeCheck size={13} /> CAC Verified</span>}{!identityVerified && !cacVerified && listing.verified && <span className="verification-basic-badge"><BadgeCheck size={13} /> Verified seller</span>}</div>{(sellerPhone || whatsapp) && <div className="listing-contact-availability"><span>Contact available</span><small>{sellerPhone ? 'Phone enabled' : 'WhatsApp enabled'}</small></div>}</section>
        <section className="listing-detail-card"><div className="listing-card-heading"><div><div className="eyebrow">TRUST</div><h2>Feedback about seller</h2></div><span className="listing-review-count">{reviews.length} review{reviews.length === 1 ? '' : 's'}</span></div>{reviews.length ? reviews.slice(0, 2).map((review) => <article className="review-row review-row-clean" key={review.id}><strong>{review.reviewer?.display_name || 'Buyer'}</strong><span>{'★'.repeat(Math.max(0, Math.min(5, Number(review.rating) || 0)))} · {new Date(review.created_at).toLocaleDateString('en-NG')}</span><p>{review.body || 'No written review.'}</p></article>) : <div className="listing-empty-feedback listing-empty-feedback-compact"><Star size={17} /><span>No reviews yet</span><small>Be the first buyer to share your experience.</small></div>}</section>
        <section className="listing-detail-card listing-safety-card"><div><strong>Shop safely on Bese26</strong><p>Meet in a public place, inspect the item, and never share your OTP or password.</p></div><button type="button" className="listing-report-button" onClick={() => setReportOpen(true)}><Flag size={14} /> Report listing</button></section>
        {similar.length > 0 && <section className="listing-similar-feed"><div className="listing-card-heading"><div><h2>Similar listings</h2><small className="listing-feed-note">More products from Bese26 sellers</small></div></div><div className="listing-similar-products">{similar.slice(0, similarVisibleCount).map((item) => <ProductCard key={item.id} listing={item} onOpen={onOpenListing} isSaved={false} onToggleSave={onToggleSave} compact />)}</div><div ref={similarSentinelRef} className="listing-similar-sentinel" aria-hidden="true" />{similarVisibleCount < similar.length && <div className="listing-feed-loading">Loading more listings…</div>}</section>}
      ...
      <div className="listing-mobile-sticky-actions"><strong>{listing.price}</strong><button type="button" className="listing-mobile-chat" onClick={() => onStartChat?.(listing, 'message')}><MessageCircle size={16} /> Chat seller</button></div>
    </div>
    {zoomed && <div className="listing-gallery-lightbox" role="dialog" aria-label="Fullscreen listing gallery" onClick={() => setZoomed(false)}><button className="icon-button lightbox-close" onClick={() => setZoomed(false)} aria-label="Close fullscreen"><X size={20} /></button>{gallery.length > 1 && <button className="gallery-control gallery-control-prev" onClick={(event) => { event.stopPropagation(); previousImage(); }} aria-label="Previous photo"><ArrowLeft size={20} /></button>}<img src={safeImage} alt={`${listing.title} fullscreen image ${activeImage + 1}`} onClick={(event) => event.stopPropagation()} />{gallery.length > 1 && <button className="gallery-control gallery-control-next" onClick={(event) => { event.stopPropagation(); nextImage(); }} aria-label="Next photo"><ArrowRight size={20} /></button>}</div>}
