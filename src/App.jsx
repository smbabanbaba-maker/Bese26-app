...
function ListingModal({ listing, user, onClose, isSaved, onToggleSave, onDemoAction, onAuthRequired, onStartChat, onEditListing }) {
  ...
  const share = async () => { const url = `https://bese26.shop/listing/${listing.id}`; try { if (navigator.share) await navigator.share({ title: listing.title, text: listing.description, url }); else { await navigator.clipboard?.writeText(url); onDemoAction?.('Listing link copied.'); } } catch (_) {} };
