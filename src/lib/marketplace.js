...
export async function fetchActiveAdCampaigns({ placement = 'home_banner' } = {}) {
  if (!supabase) return [];
  const { data, error } = await supabase.from('ad_campaigns').select('id,title,body,image_url,image_only,cta_label,cta_target,placement,status,priority,starts_at,ends_at').eq('placement', placement).eq('status', 'active').lte('starts_at', new Date().toISOString()).or(`ends_at.is.null,ends_at.gt.${new Date().toISOString()}`).order('priority', { ascending: false }).order('created_at', { ascending: false }).limit(12);
  if (error) return [];
  return data || [];
}
...
export async function fetchBusinessDirectory(search = '') {
  failIfUnavailable();
  let query = supabase.from('business_profiles').select('profile_id,business_name,business_handle,business_type,logo_path,category,description,country,state,city,delivery_available,pickup_available,is_verified,verification_expires_at,is_active,public_contact').eq('is_active', true).not('business_handle', 'is', null).neq('business_handle', '').order('business_name').limit(60);
...
export async function getBusinessProfile(userId) {
  ...
  if (error && /contact_preference|verification_status|verified_at|suspended_at|suspension_reason/i.test(error.message || '')) ({ data, error } = await supabase.from('business_profiles').select(legacyFields).eq('profile_id', userId).maybeSingle());
  if (error) throw error;
  return data;
}
...
export async function saveBusinessProfile(userId, values) {
  ...
  if (error && /contact_preference|verification_status|verified_at|suspended_at|suspension_reason/i.test(error.message || '')) { const { contact_preference: _ignoredContactPreference, ...legacyPayload } = payload; ({ data, error } = await supabase.from('business_profiles').upsert(legacyPayload).select(legacyFields).single()); }
  if (error) throw error;
  return data;
}
