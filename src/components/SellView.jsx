...
export default function SellView({ user, onAuthRequired, onDemoAction, onOpenSubscription, onNavigate, initialListing = null, initialDraft = null }) {
  ...
  const renderDetails = () => <section id="sell-details" className="sell-work-card"><div className="sell-work-heading"><span className="sell-step-icon lavender"><Package size={19} /></span><div><div className="eyebrow">ITEM DETAILS</div><h2>Tell buyers about it</h2><p>Choose what you are selling and we will show only the useful fields.</p></div></div><div className="sell-category-guide"><span><Sparkles size={15} /></span><div><strong>{form.category === 'Electronics' && form.subcategory === 'Phones' ? 'Phone listing fields ready' : `${form.category} listing fields ready`}</strong><small>{categoryGuidance}. We only show what buyers need to make a decision.</small></div></div><div className="sell-category-picker" aria-label="Choose a listing category">{Object.keys(categoryGroups).map((category) => <button type="button" key={category} className={form.category === category ? 'selected' : ''} onClick={() => { update('category', category); update('subcategory', categoryGroups[category][0]); }}><span>{category.slice(0, 2).toUpperCase()}</span>{category}</button>)}</div><div className="sell-form-grid"><Field label="Title*" value={form.title} onChange={(value) => update('title', value)} placeholder="e.g. iPhone 15 Pro Max 256GB" wide /><div className="sell-field sell-category-select-field"><span>Category*</span><select value={form.category} onChange={(event) => { const category = event.target.value; update('category', category); update('subcategory', categoryGroups[category][0]); }}>{Object.keys(categoryGroups).map((category) => <option key={category}>{category}</option>)}</select></div><div className="sell-field"><span>Subcategory</span><select value={form.subcategory} onChange={(event) => update('subcategory', event.target.value)}>{subcategories.map((item) => <option key={item}>{item}</option>)}</select></div>{categoryNeedsCondition && <div className="sell-field"><span>Condition*</span><select value={form.condition} onChange={(event) => update('condition', event.target.value)}>{['New', 'Like New', 'Used — Excellent', 'Used — Good', 'Used — Fair', 'Refurbished', 'For Parts / Repair', 'Not Applicable'].map((item) => <option key={item}>{item}</option>)}</select></div>}{fields.map(([key, label, placeholder, options]) => <Field key={key} label={label} value={form[key]} onChange={(value) => update(key, value)} placeholder={placeholder} options={options} />)}<Field label="Description*" value={form.description} onChange={(value) => update('description', value)} placeholder="Tell buyers what makes this item or service worth choosing..." type="textarea" wide /></div></section>;
