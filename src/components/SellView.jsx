...
export default function SellView({ user, isAdmin = false, onAuthRequired, onDemoAction, onOpenSubscription, onNavigate, initialListing = null, initialDraft = null }) {
  ...
  useEffect(() => {
    ...
    setForm((current) => ({ ...current, sellerName: businessProfile.business_name || current.sellerName }));
  ...
  const planUsageNote = !editMode && entitlement ? <div className={`sell-plan-usage ${!isAdmin && entitlement.free_posts_remaining === 0 ? 'exhausted' : ''}`}><div><strong>{isAdmin ? 'Admin access · unlimited listings' : entitlement.is_paid ? `${entitlement.plan_key} plan · ${entitlement.free_posts_used} of ${entitlement.listing_limit} active listings used` : `${entitlement.free_posts_used} of ${entitlement.listing_limit} active listings used`}</strong><small>{isAdmin ? 'Post as many listings as needed.' : entitlement.is_paid ? `${Math.max(entitlement.listing_limit - entitlement.free_posts_used, 0)} active listings remaining` : entitlement.free_posts_remaining ? `${entitlement.free_posts_remaining} active listing slot${entitlement.free_posts_remaining === 1 ? '' : 's'} remaining` : 'Your Free plan has reached its active listing limit.'}</small></div>{!isAdmin && entitlement.free_posts_remaining === 0 && !entitlement.is_paid && <button type="button" className="text-action" onClick={onOpenSubscription}>View plans <ArrowRight size={14} /></button>}</div> : null;
