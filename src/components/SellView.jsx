...
export default function SellView({ user, isAdmin = false, onAuthRequired, onDemoAction, onOpenSubscription, onNavigate, initialListing = null, initialDraft = null }) {
  ...
  const publish = async () => {
    ...
    if (!editMode && !isAdmin && entitlement) {
      const paidExpired = entitlement.plan_key !== 'free' && !entitlement.is_paid;
      const remaining = entitlement.is_paid ? Number(entitlement.listing_limit || 0) - Number(entitlement.free_posts_used || 0) : Number(entitlement.free_posts_remaining ?? 0);
      if (paidExpired || remaining <= 0) {
        setErrors([paidExpired ? 'Your subscription has expired. Get a subscription to publish another listing.' : 'Your active listing limit has finished. Get a subscription to publish more listings.']);
        onOpenSubscription?.();
        return;
      }
    }
