...
export default function SellView({ user, isAdmin = false, onAuthRequired, onDemoAction, onOpenSubscription, onNavigate, initialListing = null, initialDraft = null }) {
  ...
  const planUsageNote = !editMode && entitlement ? <div className={`sell-plan-usage ${!isAdmin && (entitlement.free_posts_remaining === 0 || (entitlement.plan_key !== 'free' && !entitlement.is_paid)) ? 'exhausted' : ''}`}><div><strong>{isAdmin ? 'Admin access · unlimited listings' : entitlement.plan_key !== 'free' && !entitlement.is_paid ? `${entitlement.plan_key} plan expired · Free limits now apply` : entitlement.is_paid ? `${entitlement.plan_key} plan · ${entitlement.free_posts_used} of ${entitlement.listing_limit} active listings used` : `${entitlement.free_posts_used} of ${entitlement.listing_limit} active listings used`}</strong><small>{isAdmin ? 'Post as many listings as needed.' : entitlement.plan_key !== 'free' && !entitlement.is_paid ? 'Your paid access has ended. Subscribe again to restore your plan capacity and tools.' : entitlement.is_paid ? `${Math.max(entitlement.listing_limit - entitlement.free_posts_used, 0)} active listings remaining` : entitlement.free_posts_remaining ? `${entitlement.free_posts_remaining} active listing slot${entitlement.free_posts_remaining === 1 ? '' : 's'} remaining` : 'Your Free plan has reached its active listing limit.'}</small></div>{!isAdmin && ((entitlement.plan_key !== 'free' && !entitlement.is_paid) || (entitlement.free_posts_remaining === 0 && !entitlement.is_paid)) && <button type="button" className="text-action" onClick={onOpenSubscription}>{entitlement.plan_key !== 'free' ? 'Subscribe again' : 'View plans'} <ArrowRight size={14} /></button>}</div> : null;
