...
function BusinessProfilePage({ user, onBack, onNotice }) {
  const [form, setForm] = useState({ business_name: '', category: '', description: '', phone: '', email: '', country: 'Nigeria', state: '', city: '', address: '', website: '', registration_number: '' });
  const [verified, setVerified] = useState(false);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  useEffect(() => { let mounted = true; getBusinessProfile(user.id).then((data) => { if (!mounted) return; if (data) { setForm((current) => ({ ...current, ...data })); setVerified(Boolean(data.is_verified)); } }).catch((requestError) => mounted && setError(requestError.message || 'Could not load business profile.')).finally(() => mounted && setLoading(false)); return () => { mounted = false; }; }, [user.id]);
  const update = (key, value) => setForm((current) => ({ ...current, [key]: value }));
  const save = async (event) => { event.preventDefault(); setSaving(true); setError(''); try { const data = await saveBusinessProfile(user.id, form); setForm((current) => ({ ...current, ...data })); setVerified(Boolean(data.is_verified)); onNotice('Business profile saved.'); } catch (requestError) { setError(requestError.message || 'Could not save business profile.'); } finally { setSaving(false); } };
  ...
  return <div className="profile-subpage"><SubpageHeader title="Business Profile" eyebrow="SELLER TOOLS" onBack={onBack} />{error && <div className="auth-status error"><AlertCircle size={15} /> {error}</div>}<div className="future-card"><Store size={20} /><div><strong>{verified ? 'Verified business record' : 'Business profile not verified'}</strong><p>{verified ? 'This profile is verified by the Bese26 business record.' : 'You can save your business details now. Verification remains read-only until the real review workflow is completed.'}</p></div></div><form className="settings-card profile-form-card" onSubmit={save}><label>Business name<input required value={form.business_name} onChange={(event) => update('business_name', event.target.value)} placeholder="Your registered business name" /></label><label>Business category<input value={form.category || ''} onChange={(event) => update('category', event.target.value)} placeholder="e.g. Agriculture" /></label><label>Email<input type="email" value={form.email || ''} onChange={(event) => update('email', event.target.value)} /></label><label>Phone<input value={form.phone || ''} onChange={(event) => update('phone', event.target.value)} /></label><label>Country<input value={form.country || 'Nigeria'} onChange={(event) => update('country', event.target.value)} /></label><label>State<input value={form.state || ''} onChange={(event) => update('state', event.target.value)} /></label><label>City<input value={form.city || ''} onChange={(event) => update('city', event.target.value)} /></label><label>Address<input value={form.address || ''} onChange={(event) => update('address', event.target.value)} /></label><label>Website<input type="url" value={form.website || ''} onChange={(event) => update('website', event.target.value)} placeholder="https://" /></label><label>Registration number<input value={form.registration_number || ''} onChange={(event) => update('registration_number', event.target.value)} /></label><label className="full-field">Description<textarea value={form.description || ''} onChange={(event) => update('description', event.target.value)} /></label><button className="primary-button full-width" disabled={saving}>{saving ? 'Saving…' : 'Save business profile'} <Check size={15} /></button></form></div>;
}
...
function VerificationPage({ profile, onBack }) {
...
export default function ProfileView({ user, onAuthRequired, onSignOut, onDemoAction, isDark, onToggleTheme, onNavigate, onCreateListing, onEditListing, onOpenListing, onToggleSave, isAdmin = false, onOpenAdmin, onOpenSubscription, isActive = true }) {
  ...
  if (subPage === 'business') return <BusinessProfilePage user={user} onBack={() => setSubPage('main')} onNotice={onDemoAction} />;
  ...
  if (subPage === 'verification') return <VerificationPage profile={profileRecord} onBack={() => setSubPage('main')} />;
  ...
  const paymentItems = [{ label: 'Wallet', description: 'No balance shown until connected', icon: WalletCards, page: 'wallet', tone: 'coral' }, { label: 'Subscription', description: 'View seller plans and free posts', icon: Tag, page: 'subscription', tone: 'lavender' }, { label: 'Boosting', description: 'Promotion service not connected', icon: ArrowRight, page: 'boosting', tone: 'gold' }, { label: 'Payment History', description: 'Transaction ledger not connected', icon: FileText, page: 'payment-history', tone: 'navy' }];
