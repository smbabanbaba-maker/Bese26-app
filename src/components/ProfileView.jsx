...
import { fetchMyListings, fetchSellerStats } from '../lib/marketplace';
...
function SimpleInfoPage({ page, onBack, onDemoAction, isDark, onToggleTheme, user, onAuthRequired, onSignOut }) {
  ...
  if (page.key === 'security') return <div className="profile-subpage">{back}<div className="future-card"><LockKeyhole size={22} /><div><strong>Email sign-in is enabled.</strong><p>Use a one-time email code to protect access to your bese26 account.</p></div></div><div className="settings-card security-list"><button onClick={() => onDemoAction('Email OTP settings will be available here.') }><span><LockKeyhole size={16} />Email OTP</span><span className="future-pill">Enabled</span><ChevronRight size={15} /></button></div></div>;
  ...
  if (page.key === 'report') return <div className="profile-subpage">{back}<div className="report-form"><label>What happened?<select defaultValue="Bug"><option>Bug</option><option>Incorrect listing</option><option>Suspicious seller</option><option>Harassment</option><option>Spam</option><option>Technical issue</option><option>Other</option></select></label><label>Describe the problem<textarea value={reportDescription} onChange={(event) => setReportDescription(event.target.value)} placeholder="Tell us what happened..." /></label><button className="primary-button full-width" onClick={() => { if (!reportDescription.trim()) { onDemoAction('Please describe the problem before submitting.'); return; } onDemoAction('Report submitted successfully.'); setReportDescription(''); }}>Submit report <ArrowRight size={16} /></button></div></div>;
...
export default function ProfileView({ user, onAuthRequired, onSignOut, onDemoAction, isDark, onToggleTheme, onNavigate, onCreateListing, isActive = true }) {
  ...
  useEffect(() => {
    let mounted = true;
    if (!user) { setStats(null); return undefined; }
    setStatsLoading(true);
    fetchSellerStats(user.id).then((nextStats) => mounted && setStats(nextStats)).catch((error) => mounted && onDemoAction(error.message || 'Could not load profile statistics.')).finally(() => mounted && setStatsLoading(false));
    return () => { mounted = false; };
  }, [user, onDemoAction]);
  const displayName = user?.user_metadata?.display_name || user?.email?.split('@')[0] || 'Your bese26 profile';
  const username = user?.user_metadata?.username ? `@${user.user_metadata.username}` : user?.email || 'Sign in to personalize your profile';
  const profileInitials = displayName.split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]).join('').toUpperCase() || 'BE';
  const requiresAccount = ['listings', 'sold', 'reviews', 'personal', 'analytics', 'promote', 'promotions'];
