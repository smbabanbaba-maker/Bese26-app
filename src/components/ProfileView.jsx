...
function PromotePage({ onBack, onDemoAction }) {
  ...
  return <div className="profile-subpage"><SubpageHeader title="Promote Listings" eyebrow="GET NOTICED" onBack={onBack} /><div className="promote-intro"><div className="promote-intro-icon"><Sparkles size={22} /></div><div><h2>Put your best listing in front.</h2><p>Choose a demo promotion package. No payment will be processed.</p></div></div><div className="promotion-options">{options.map(({ title, price, detail, icon: Icon }) => <button className="promotion-option" key={title} onClick={() => onDemoAction(`${title} selected — demo confirmation shown.`)}><span className="promotion-option-icon"><Icon size={18} /></span><span><strong>{title}</strong><small>{detail}</small></span><b>{price}</b><ChevronRight size={15} /></button>)}</div><div className="prototype-note">Promotions are prototype-only and do not process real payments.</div></div>;
}
