...
export default function AuthPanel({ onClose, onAuthenticated, reason = '' }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (mode === 'signup' && !data.session) { await sendEmailOtp(form.email); setOtpSent(true); setStatus({ type: 'success', message: 'Account created. We sent a 6-digit verification code to your email. Enter it below to finish registration.' }); }
  ...
  const renderSignup = () => <div className="auth-form-panel auth-form-face-content">
    ...
    {status.message && <div className={`auth-status ${status.type}`}><CheckCircle2 size={15} /><span>{status.message}</span></div>}
    {isSupabaseConfigured && <><GoogleButton onClick={continueWithGoogle} disabled={loading} /><div className="auth-divider"><span>or use email</span></div></>}
    {otpSent ? <div className="auth-otp-box"><div className="auth-divider"><span>VERIFY YOUR EMAIL</span></div><p className="auth-panel-copy">Your account is almost ready. Enter the 6-digit code sent to <strong>{form.email}</strong> to complete registration.</p><form className="auth-otp-form" onSubmit={verifyOtp}><label><span><Mail size={14} /> 6-digit verification code</span><input inputMode="numeric" pattern="[0-9]{6}" maxLength={6} value={otp} onChange={(event) => setOtp(event.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="123456" autoComplete="one-time-code" required /></label><button type="submit" className="primary-button auth-submit" disabled={loading}>{loading ? 'Verifying…' : 'Verify email & finish'}</button><button type="button" className="auth-forgot" onClick={sendOtp} disabled={loading}>Didn’t receive the code? Send again</button></form></div> : <form onSubmit={submit} className="auth-form auth-signup-form">
      ...
      <button type="submit" className="primary-button auth-submit" disabled={loading}>{loading ? 'Please wait…' : 'Create account & verify email'}</button>
    </form>}
